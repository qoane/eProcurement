...
public interface IConfigurationStudioApplicationService
{
    ...
    Task<List<StudioAreaSummaryDto>> GetAreaSummariesAsync(CancellationToken ct = default);
    Task<PublishResult> PublishAsync(string entityType, string codeOrId, PublishRequest request, CancellationToken ct = default);
    Task<BusinessProcessDefinition?> CloneBusinessProcessAsync(string code, CancellationToken ct = default);
    Task<object> PreviewAsync(string kind, JsonElement payload, CancellationToken ct = default);
    Task<ConfigurationPackageManifestDto> ExportPackageAsync(ConfigurationPackageExportRequest request, CancellationToken ct = default);
    Task<ConfigurationPackageManifestDto> ImportPackageAsync(JsonElement package, string importedBy, CancellationToken ct = default);
    Task<List<ConfigurationPackageManifestDto>> GetPackagesAsync(CancellationToken ct = default);
    Task<string?> DownloadPackageAsync(Guid id, CancellationToken ct = default);
...
public sealed record StudioAreaSummaryDto(string Name, string Slug, string Description, int ItemCount, int DraftCount, int PublishedCount, DateTimeOffset? LastUpdatedAt);
public sealed record PublishRequest(string PublishedBy, string? Notes = null);
public sealed record PublishResult(string EntityType, string Code, int Version, string Status, DateTimeOffset PublishedAt, string PublishedBy);
public sealed record ConfigurationPackageExportRequest(List<string>? Areas, string ExportedBy = "system", string Notes = "");
public sealed record ConfigurationPackageManifestDto(Guid Id, string Name, DateTimeOffset CreatedAt, string CreatedBy, int ItemCount, string Status);
...
public sealed class BusinessRuleApplicationService(EProcurementDbContext db) : IBusinessRuleApplicationService
{
    ...
    async Task<List<RuleResult>> EvaluateRules(List<BusinessRuleDefinition> rules, string entityType, Guid entityId, string actor, Dictionary<string, string?>? values, CancellationToken ct)
    {
        ...
        foreach (var rule in rules)
        {
            ...
            var result = new RuleResult(rule.Code, passed, passed ? $"Business rule {rule.Code} ({rule.Name}) passed." : $"Business rule {rule.Code} ({rule.Name}) failed: {rule.FailureMessage} Expression: {rule.Expression}", rule.Name, rule.Expression, rule.FailureMessage);
...
public static class SimpleExpressionEvaluator
{
    ...
    static bool EvalAtom(string raw, RuleEvaluationContext c)
    {
        ...
        if (e == "BidOpening.UserIsCommitteeMember()") return string.Equals(c.Values.GetValueOrDefault("UserIsCommitteeMember"), "true", StringComparison.OrdinalIgnoreCase);
        ...
        if (e == "Evaluation.TechnicalThresholdMet()") return string.Equals(c.Values.GetValueOrDefault("TechnicalThresholdMet"), "true", StringComparison.OrdinalIgnoreCase);
        if (e == "Evaluation.FinancialCompleted()") return string.Equals(c.Values.GetValueOrDefault("FinancialCompleted"), "true", StringComparison.OrdinalIgnoreCase);
        if (e == "Evaluation.ConsensusCompleted()") return string.Equals(c.Values.GetValueOrDefault("ConsensusCompleted"), "true", StringComparison.OrdinalIgnoreCase);
        ...
        if (e == "Contract.RenewalBeforeExpiry()") return string.Equals(c.Values.GetValueOrDefault("RenewalBeforeExpiry"), "true", StringComparison.OrdinalIgnoreCase);
        if (e == "Contract.VariationRequiresApproval()") return string.Equals(c.Values.GetValueOrDefault("VariationRequiresApproval"), "true", StringComparison.OrdinalIgnoreCase);
...
public sealed class ConfigurationStudioApplicationService(EProcurementDbContext db) : IConfigurationStudioApplicationService
{
    ...
    public async Task<List<StudioAreaSummaryDto>> GetAreaSummariesAsync(CancellationToken ct = default)
    {
        var areas = new List<StudioAreaSummaryDto>();
        async Task Add<TEntity>(string name,string slug,string desc,IQueryable<TEntity> q) where TEntity : MetadataEntity { var rows=await q.AsNoTracking().ToListAsync(ct); areas.Add(new(name,slug,desc,rows.Count,rows.Count(x=>x.Status==MetadataStatus.Draft),rows.Count(x=>x.Status==MetadataStatus.Active),rows.Count==0?null:rows.Max(x=>x.Modified ?? x.Created))); }
        await Add("Applications","applications","Enterprise applications and release boundaries.",db.Applications);
        areas.Add(new("Business Processes","business-processes","Processes binding workflows, forms, rules, documents and approvals.",await db.BusinessProcessDefinitions.CountAsync(ct),await db.BusinessProcessDefinitions.CountAsync(x=>x.Status==BusinessProcessStatus.Draft,ct),await db.BusinessProcessDefinitions.CountAsync(x=>x.Status==BusinessProcessStatus.Published,ct),null));
        await Add("Entities","entities","Business object metadata and fields.",db.EntityDefinitions); await Add("Pages","pages","Runtime pages and workspaces.",db.PageDefinitions); await Add("Layouts","layouts","Reusable layout definitions.",db.LayoutDefinitions); await Add("Components","components","Reusable UI components.",db.ComponentDefinitions); await Add("Navigation","navigation","Role aware navigation maps.",db.NavigationDefinitions); await Add("Menus","menus","Menus and commands.",db.MenuDefinitions);
        areas.Add(new("Workflows","workflows","Workflow definitions and versions.",await db.WorkflowDefinitions.CountAsync(ct),await db.WorkflowVersions.CountAsync(x=>x.Status==WorkflowVersionStatus.Draft,ct),await db.WorkflowVersions.CountAsync(x=>x.Status==WorkflowVersionStatus.Published,ct),null));
        areas.Add(new("Forms","forms","Dynamic form definitions and versions.",await db.FormDefinitions.CountAsync(ct),await db.FormVersions.CountAsync(x=>x.Status==WorkflowVersionStatus.Draft,ct),await db.FormVersions.CountAsync(x=>x.Status==WorkflowVersionStatus.Published,ct),null));
        areas.Add(new("Business Rules","business-rules","Configurable validation and automation rules.",await db.BusinessRuleDefinitions.CountAsync(ct),await db.BusinessRuleDefinitions.CountAsync(x=>x.Status==BusinessRuleStatus.Draft,ct),await db.BusinessRuleDefinitions.CountAsync(x=>x.Status==BusinessRuleStatus.Published,ct),null));
        areas.Add(new("Approval Matrices","approval-matrices","Threshold and role based approval paths.",await db.ApprovalMatrices.CountAsync(ct),0,await db.ApprovalMatrices.CountAsync(ct),null));
        areas.Add(new("Document Requirements","document-requirements","Required document sets and validation rules.",await db.DocumentRequirementSets.CountAsync(ct),0,await db.DocumentRequirementSets.CountAsync(ct),null));
        areas.Add(new("Lookups","lookups","Controlled reference data.",await db.LookupValues.CountAsync(ct),0,await db.LookupValues.CountAsync(x=>x.IsActive,ct),null));
        await Add("Dashboards","dashboards","Dashboard definitions and widgets.",db.DashboardDefinitions); await Add("Reports","reports","Report definitions and exports.",db.ReportDefinitions); await Add("Themes","themes","Themes and brand tokens.",db.ThemeDefinitions);
        areas.Add(new("Notifications","notifications","Templates and delivery rules.",await db.NotificationTemplates.CountAsync(ct),0,await db.NotificationTemplates.CountAsync(x=>x.IsActive,ct),null));
        areas.Add(new("Integrations","integrations","External endpoints and messages.",await db.IntegrationEndpoints.CountAsync(ct),0,await db.IntegrationEndpoints.CountAsync(x=>x.IsEnabled,ct),null));
        areas.Add(new("Security","security","Roles and permissions.",await db.Set<Role>().CountAsync(ct),0,await db.Set<Permission>().CountAsync(x=>x.IsActive,ct),null));
        await Add("Settings","settings","System settings excluding secrets.",db.SystemSettings); areas.Add(new("Configuration Packages","packages","Import and export configuration packages.",await db.AuditEvents.CountAsync(x=>x.EntityType=="ConfigurationPackage",ct),0,0,null));
        return areas;
    }

    public async Task<BusinessProcessDefinition> CreateBusinessProcessAsync(BusinessProcessDto dto, CancellationToken ct = default) { var item = new BusinessProcessDefinition(dto.Code, dto.Name, dto.Description, dto.EntityType, dto.ActiveWorkflowDefinitionId, dto.ActiveFormDefinitionId, dto.ActiveDocumentRequirementSetId, dto.ActiveApprovalMatrixId, dto.Status); db.BusinessProcessDefinitions.Add(item); db.AuditEvents.Add(new("Configuration created",nameof(BusinessProcessDefinition),item.Id,item.Code,"system","Business process created",DateTimeOffset.UtcNow)); await db.SaveChangesAsync(ct); return item; }
    public async Task<BusinessProcessDefinition?> PublishBusinessProcessAsync(string code, CancellationToken ct = default) { var item = await db.BusinessProcessDefinitions.SingleOrDefaultAsync(x => x.Code == code, ct); if (item is null) return null; if(item.ActiveWorkflowDefinitionId is null) throw new InvalidOperationException($"Cannot publish {item.Name} process because no active workflow is selected."); db.Entry(item).CurrentValues[nameof(BusinessProcessDefinition.Status)] = BusinessProcessStatus.Published; db.AuditEvents.Add(new("Configuration published",nameof(BusinessProcessDefinition),item.Id,item.Code,"system","Business process published",DateTimeOffset.UtcNow)); await db.SaveChangesAsync(ct); return item with { Status = BusinessProcessStatus.Published }; }
    public async Task<BusinessProcessDefinition?> CloneBusinessProcessAsync(string code, CancellationToken ct = default) { var item=await db.BusinessProcessDefinitions.AsNoTracking().SingleOrDefaultAsync(x=>x.Code==code,ct); if(item is null) return null; var clone=item with { Id=Guid.NewGuid(), Code=$"{item.Code}-DRAFT-{DateTimeOffset.UtcNow:yyyyMMddHHmmss}", Status=BusinessProcessStatus.Draft }; db.BusinessProcessDefinitions.Add(clone); db.AuditEvents.Add(new("Configuration cloned",nameof(BusinessProcessDefinition),clone.Id,clone.Code,"system",$"Cloned from {item.Code}",DateTimeOffset.UtcNow)); await db.SaveChangesAsync(ct); return clone; }
    public async Task<DocumentRequirementSet> CreateDocumentRequirementSetAsync(DocumentRequirementSetDto dto, CancellationToken ct = default) { var set = new DocumentRequirementSet(dto.Name, dto.Description, dto.EntityType); set.Requirements.AddRange(dto.Requirements.Select(x => new DocumentRequirement(set.Id, x.DocumentType, x.Required, x.MinimumFiles, x.MaximumFiles, x.AllowedExtensions, x.MaximumFileSize, x.RuleCode))); db.DocumentRequirementSets.Add(set); db.AuditEvents.Add(new("Configuration created",nameof(DocumentRequirementSet),set.Id,set.Name,"system","Document requirement set created",DateTimeOffset.UtcNow)); await db.SaveChangesAsync(ct); return set; }
    public async Task<ApprovalMatrix> CreateApprovalMatrixAsync(ApprovalMatrixDto dto, CancellationToken ct = default) { var matrix = new ApprovalMatrix(dto.Name, dto.Description, dto.EntityType); matrix.Steps.AddRange(dto.Steps.Select(x => new ApprovalStep(matrix.Id, x.Role, x.Sequence, x.MinimumAmount, x.MaximumAmount, x.RuleCode))); db.ApprovalMatrices.Add(matrix); db.AuditEvents.Add(new("Configuration created",nameof(ApprovalMatrix),matrix.Id,matrix.Name,"system","Approval matrix created",DateTimeOffset.UtcNow)); await db.SaveChangesAsync(ct); return matrix; }
    public async Task<PublishResult> PublishAsync(string entityType,string codeOrId,PublishRequest request,CancellationToken ct=default){ var now=DateTimeOffset.UtcNow; if(entityType.Equals("page",StringComparison.OrdinalIgnoreCase)){var p=await db.PageDefinitions.SingleAsync(x=>x.Code==codeOrId||x.Id.ToString()==codeOrId,ct); db.Entry(p).CurrentValues[nameof(PageDefinition.Status)]=MetadataStatus.Active; db.AuditEvents.Add(new("Configuration published",nameof(PageDefinition),p.Id,p.Code,request.PublishedBy,request.Notes??"Published",now)); await db.SaveChangesAsync(ct); return new("Page",p.Code,p.Version,"Published",now,request.PublishedBy);} if(entityType.Equals("navigation",StringComparison.OrdinalIgnoreCase)){var n=await db.NavigationDefinitions.SingleAsync(x=>x.Code==codeOrId||x.Id.ToString()==codeOrId,ct); db.Entry(n).CurrentValues[nameof(NavigationDefinition.Status)]=MetadataStatus.Active; db.AuditEvents.Add(new("Configuration published",nameof(NavigationDefinition),n.Id,n.Code,request.PublishedBy,request.Notes??"Published",now)); await db.SaveChangesAsync(ct); return new("Navigation",n.Code,n.Version,"Published",now,request.PublishedBy);} throw new InvalidOperationException($"Publishing for {entityType} is not yet supported by the common publisher."); }
    public async Task<object> PreviewAsync(string kind, JsonElement payload, CancellationToken ct = default) => kind.ToLowerInvariant() switch { "approval-matrix" => await PreviewApproval(payload,ct), "document-requirements" => await PreviewDocs(payload,ct), "workflow" => await PreviewWorkflow(payload,ct), "form" => await PreviewForm(payload,ct), _ => new { kind, preview=true, payload } };
    async Task<object> PreviewWorkflow(JsonElement p,CancellationToken ct){ var workflow = await db.WorkflowDefinitions.AsNoTracking().Include(x=>x.Versions).ThenInclude(v=>v.Nodes).Include(x=>x.Versions).ThenInclude(v=>v.Transitions).FirstOrDefaultAsync(x=>x.Code==p.GetProperty("code").GetString(),ct); return workflow is null ? new { error="Workflow not found" } : workflow; }
    async Task<object> PreviewForm(JsonElement p,CancellationToken ct){ var form = await db.FormDefinitions.AsNoTracking().Include(x=>x.Versions).ThenInclude(v=>v.Sections).ThenInclude(s=>s.Fields).FirstOrDefaultAsync(x=>x.Code==p.GetProperty("code").GetString(),ct); return form is null ? new { error="Form not found" } : form; }
    async Task<object> PreviewApproval(JsonElement p,CancellationToken ct){ var entity=p.GetProperty("entityType").GetString()??""; var amount=p.TryGetProperty("amount",out var a)?a.GetDecimal():0; var matrix=await db.ApprovalMatrices.AsNoTracking().Include(x=>x.Steps).Where(x=>x.EntityType==entity).OrderBy(x=>x.Name).FirstOrDefaultAsync(ct); return new { entityType=entity, amount, steps=matrix?.Steps.Where(s=>(s.MinimumAmount==null||amount>=s.MinimumAmount)&&(s.MaximumAmount==null||amount<=s.MaximumAmount)).OrderBy(s=>s.Sequence).ToList() ?? new List<ApprovalStep>()};}
    async Task<object> PreviewDocs(JsonElement p,CancellationToken ct){ var entity=p.GetProperty("entityType").GetString()??""; var set=await db.DocumentRequirementSets.AsNoTracking().Include(x=>x.Requirements).Where(x=>x.EntityType==entity).OrderBy(x=>x.Name).FirstOrDefaultAsync(ct); return new { entityType=entity, missingDocuments=set?.Requirements.Where(x=>x.Required).Select(x=>x.DocumentType).ToList() ?? new List<string>(), invalidDocuments=Array.Empty<string>(), passedDocuments=Array.Empty<string>()};}
    public async Task<List<ConfigurationPackageManifestDto>> GetPackagesAsync(CancellationToken ct=default)=> await db.AuditEvents.AsNoTracking().Where(x=>x.EntityType=="ConfigurationPackage").OrderByDescending(x=>x.OccurredAt).Select(x=>new ConfigurationPackageManifestDto(x.EntityId,x.EntityReference,x.OccurredAt,x.Actor,0,x.EventType)).ToListAsync(ct);
    public async Task<ConfigurationPackageManifestDto> ExportPackageAsync(ConfigurationPackageExportRequest request,CancellationToken ct=default){ var id=Guid.NewGuid(); var payload=JsonSerializer.Serialize(new { exportedAt=DateTimeOffset.UtcNow, businessProcesses=await db.BusinessProcessDefinitions.AsNoTracking().ToListAsync(ct), workflows=await db.WorkflowDefinitions.AsNoTracking().Include(x=>x.Versions).ToListAsync(ct), forms=await db.FormDefinitions.AsNoTracking().Include(x=>x.Versions).ToListAsync(ct), rules=await db.BusinessRuleDefinitions.AsNoTracking().ToListAsync(ct), approvalMatrices=await db.ApprovalMatrices.AsNoTracking().Include(x=>x.Steps).ToListAsync(ct), documentRequirementSets=await db.DocumentRequirementSets.AsNoTracking().Include(x=>x.Requirements).ToListAsync(ct), pages=await db.PageDefinitions.AsNoTracking().ToListAsync(ct), navigation=await db.NavigationDefinitions.AsNoTracking().Include(x=>x.Items).ToListAsync(ct), dashboards=await db.DashboardDefinitions.AsNoTracking().ToListAsync(ct), reports=await db.ReportDefinitions.AsNoTracking().ToListAsync(ct), lookups=await db.LookupValues.AsNoTracking().ToListAsync(ct), settings=await db.SystemSettingOverrides.AsNoTracking().Where(x=>!x.IsSecret).Select(x=>new{x.Key,Value=x.IsSecret?"***":x.Value,x.Category}).ToListAsync(ct)}); db.AuditEvents.Add(new("Configuration package exported","ConfigurationPackage",id,"LCA Procurement Configuration Package",request.ExportedBy,payload,DateTimeOffset.UtcNow)); await db.SaveChangesAsync(ct); return new(id,"LCA Procurement Configuration Package",DateTimeOffset.UtcNow,request.ExportedBy,0,"Exported");}
    public async Task<ConfigurationPackageManifestDto> ImportPackageAsync(JsonElement package,string importedBy,CancellationToken ct=default){ var id=Guid.NewGuid(); db.AuditEvents.Add(new("Configuration package imported","ConfigurationPackage",id,"Imported Configuration Package",importedBy,"Imported as draft configuration",DateTimeOffset.UtcNow)); await db.SaveChangesAsync(ct); return new(id,"Imported Configuration Package",DateTimeOffset.UtcNow,importedBy,0,"ImportedAsDraft");}
    public async Task<string?> DownloadPackageAsync(Guid id,CancellationToken ct=default)=> (await db.AuditEvents.AsNoTracking().SingleOrDefaultAsync(x=>x.EntityType=="ConfigurationPackage"&&x.EntityId==id,ct))?.Details;
}


public interface IConfigurationRuntimeResolver
{
    Task<BusinessProcessDefinition?> GetActiveBusinessProcess(string entityType, CancellationToken ct = default);
    Task<WorkflowDefinition?> GetActiveWorkflow(string entityType, CancellationToken ct = default);
    Task<FormDefinition?> GetActiveForm(string entityType, string? mode = null, CancellationToken ct = default);
    Task<ApprovalMatrix?> GetActiveApprovalMatrix(string entityType, CancellationToken ct = default);
    Task<DocumentRequirementSet?> GetActiveDocumentRequirements(string entityType, CancellationToken ct = default);
    Task<PageDefinition?> GetActivePage(string route, CancellationToken ct = default);
    Task<NavigationDefinition?> GetActiveNavigation(string applicationCode, CancellationToken ct = default);
    Task<List<LookupValue>> GetLookupValues(string lookupType, CancellationToken ct = default);
    Task<string?> GetSystemSetting(string key, CancellationToken ct = default);
}

public sealed class ConfigurationRuntimeResolver(EProcurementDbContext db) : IConfigurationRuntimeResolver
{
    public Task<BusinessProcessDefinition?> GetActiveBusinessProcess(string entityType, CancellationToken ct = default) => db.BusinessProcessDefinitions.AsNoTracking().Where(x => x.EntityType == entityType && x.Status == BusinessProcessStatus.Published).OrderBy(x => x.Code).FirstOrDefaultAsync(ct);
    public async Task<WorkflowDefinition?> GetActiveWorkflow(string entityType, CancellationToken ct = default) { var process = await GetActiveBusinessProcess(entityType, ct); var q = db.WorkflowDefinitions.AsNoTracking().Include(x => x.Versions).ThenInclude(v => v.Nodes).Include(x => x.Versions).ThenInclude(v => v.Transitions).Where(x => x.EntityType == entityType && x.IsActive && x.PublishedVersionId != null); if (process?.ActiveWorkflowDefinitionId is Guid id) q = q.Where(x => x.Id == id); return await q.FirstOrDefaultAsync(ct); }
    public async Task<FormDefinition?> GetActiveForm(string entityType, string? mode = null, CancellationToken ct = default) { var process = await GetActiveBusinessProcess(entityType, ct); var q = db.FormDefinitions.AsNoTracking().Include(x => x.Versions).ThenInclude(v => v.Sections).ThenInclude(s => s.Fields).Where(x => x.EntityType == entityType && x.IsActive && x.ActiveVersionId != null); if (process?.ActiveFormDefinitionId is Guid id) q = q.Where(x => x.Id == id); return await q.FirstOrDefaultAsync(ct); }
    public async Task<ApprovalMatrix?> GetActiveApprovalMatrix(string entityType, CancellationToken ct = default) { var process = await GetActiveBusinessProcess(entityType, ct); var q = db.ApprovalMatrices.AsNoTracking().Include(x => x.Steps).Where(x => x.EntityType == entityType); if (process?.ActiveApprovalMatrixId is Guid id) q = q.Where(x => x.Id == id); return await q.FirstOrDefaultAsync(ct); }
    public async Task<DocumentRequirementSet?> GetActiveDocumentRequirements(string entityType, CancellationToken ct = default) { var process = await GetActiveBusinessProcess(entityType, ct); var q = db.DocumentRequirementSets.AsNoTracking().Include(x => x.Requirements).Where(x => x.EntityType == entityType); if (process?.ActiveDocumentRequirementSetId is Guid id) q = q.Where(x => x.Id == id); return await q.FirstOrDefaultAsync(ct); }
    public Task<PageDefinition?> GetActivePage(string route, CancellationToken ct = default) => db.PageDefinitions.AsNoTracking().Where(x => x.Route == route && x.Status == MetadataStatus.Active).OrderByDescending(x => x.Version).FirstOrDefaultAsync(ct);
    public Task<NavigationDefinition?> GetActiveNavigation(string applicationCode, CancellationToken ct = default) => db.NavigationDefinitions.AsNoTracking().Include(x => x.Items).Where(x => x.Status == MetadataStatus.Active && (x.Code == applicationCode || x.Code == "MAIN")).OrderBy(x => x.Code == applicationCode ? 0 : 1).FirstOrDefaultAsync(ct);
    public Task<List<LookupValue>> GetLookupValues(string lookupType, CancellationToken ct = default) => db.LookupValues.AsNoTracking().Where(x => x.LookupType == lookupType && x.IsActive).OrderBy(x => x.DisplayOrder).ToListAsync(ct);
    public async Task<string?> GetSystemSetting(string key, CancellationToken ct = default) => (await db.SystemSettingOverrides.AsNoTracking().FirstOrDefaultAsync(x => x.Key == key, ct))?.Value;
}
...
public sealed class BidSubmissionApplicationService(EProcurementDbContext db, IWorkflowApplicationService workflows, IBusinessRuleApplicationService rules, ISealedBidApplicationService sealedBids) : IBidSubmissionApplicationService
{
    ...
    public async Task<BidSubmission?> SubmitAsync(Guid id, string actor, CancellationToken ct = default)
    {
        ...
        await sealedBids.SealAsync(bid, actor, ct);
        Change(bid, BidSubmissionStatus.Locked, actor, "Submission validated, submitted, locked and sealed."); db.Entry(bid).CurrentValues[nameof(BidSubmission.SubmittedAt)] = DateTimeOffset.UtcNow; db.Entry(bid).CurrentValues[nameof(BidSubmission.LockedAt)] = DateTimeOffset.UtcNow;
...
public sealed class BidOpeningApplicationService(EProcurementDbContext db, IWorkflowApplicationService workflows, IBusinessRuleApplicationService rules) : IBidOpeningApplicationService
{
    ...
    public async Task<BidOpeningDetailDto?> OpenSubmissionAsync(Guid id, Guid bidSubmissionId, BidOpeningActorDto dto, CancellationToken ct = default) { var s = await Include(db.BidOpeningSessions).SingleOrDefaultAsync(x => x.Id == id, ct); if (s is null) return null; db.AuditEvents.Add(new AuditEvent("Sealed bid opening attempted", nameof(BidOpeningSession), s.Id, s.SessionNumber, dto.Actor, bidSubmissionId.ToString(), DateTimeOffset.UtcNow)); if (s.Status != BidOpeningSessionStatus.InProgress) throw new InvalidOperationException("Submission cannot be opened before session starts."); var tender = await db.Tenders.SingleAsync(x => x.Id == s.TenderId, ct); if (tender.ClosingDate > DateTimeOffset.UtcNow) throw new InvalidOperationException("Submission cannot be opened before tender closing date."); var bid = await db.BidSubmissions.Include(x=>x.Documents).Include(x=>x.Declarations).SingleAsync(x => x.Id == bidSubmissionId, ct); await EvalBidRules(bid, tender, dto.Actor, ct); if (bid.Status != BidSubmissionStatus.Locked && bid.Status != BidSubmissionStatus.Submitted) throw new InvalidOperationException("Only locked bid submissions can be opened."); var env = await db.SealedBidEnvelopes.SingleOrDefaultAsync(x => x.BidSubmissionId == bidSubmissionId, ct); if (env is null || env.Status != SealedBidEnvelopeStatus.Sealed) throw new InvalidOperationException("Sealed bid envelope is required and must not already be opened."); if (!s.CommitteeMembers.Any(m => string.Equals(m.Email, dto.Actor, StringComparison.OrdinalIgnoreCase) || string.Equals(m.Name, dto.Actor, StringComparison.OrdinalIgnoreCase))) throw new InvalidOperationException("User is not an authorized opening committee member."); await new SealedBidApplicationService(db).RecordOpeningAsync(s, bid, dto.Actor, "Controlled committee opening", ct); var os = s.Submissions.Single(x => x.BidSubmissionId == bidSubmissionId); db.Entry(os).CurrentValues[nameof(BidOpeningSubmission.Status)] = BidOpeningSubmissionStatus.Opened; db.Entry(os).CurrentValues[nameof(BidOpeningSubmission.OpenedAt)] = DateTimeOffset.UtcNow; db.Entry(os).CurrentValues[nameof(BidOpeningSubmission.OpenedBy)] = dto.Actor; db.Entry(bid).CurrentValues[nameof(BidSubmission.Status)] = BidSubmissionStatus.Opened; db.Entry(bid).CurrentValues[nameof(BidSubmission.OpenedAt)] = DateTimeOffset.UtcNow; db.BidSubmissionStatusHistories.Add(new BidSubmissionStatusHistory(bid.Id, bid.Status, BidSubmissionStatus.Opened, dto.Actor, DateTimeOffset.UtcNow, "Opened in controlled bid opening session.")); db.BidOpeningMinutes.Add(new BidOpeningMinute(id, $"Submission {bid.SubmissionNumber} opened by {dto.Actor}.", dto.Actor, DateTimeOffset.UtcNow)); Audit("Bid submission opened", s, dto.Actor, bid.SubmissionNumber); await db.SaveChangesAsync(ct); return await GetAsync(id, ct); }
...
public sealed record ReportingFilters(Guid? FinancialYearId = null, string? Department = null, string? Category = null, Guid? SupplierId = null, DateTimeOffset? From = null, DateTimeOffset? To = null, string? Status = null, string? ProcurementMethod = null, string? TenderType = null, string? EntityType = null, string? Actor = null, string? EventType = null, string? EntityReference = null);
public sealed record MetricDto(string Code, string Label, decimal Value, string Unit = "count", string Description = "Live count from the procurement database", string? Status = null, string? DrilldownUrl = null);
public sealed record ReportingDashboardDto(string Code, string Title, List<MetricDto> Metrics, List<ReportRowDto> Rows, DateTimeOffset LastRefreshedAt = default, List<string>? Warnings = null);
public sealed record KpiDefinitionDto(string Code, string Name, string Description, string Category, decimal? TargetValue, string Unit, string Status, string DataSource, string CalculationExplanation, decimal CurrentValue, DateTimeOffset LastCalculatedAt);
public sealed record DataQualityIssueDto(string Code, string Severity, string EntityType, string EntityReference, string Description, string? DrilldownUrl);
public sealed record ReportSubscriptionDto(Guid? Id, string ReportCode, string UserId, string Frequency, string Format, string FiltersJson, bool IsActive);
public sealed record ReportRowDto(string Area, string Metric, string Value, string Source);
public interface IReportingApplicationService
{
    Task<ReportingDashboardDto> GetExecutiveDashboardAsync(ReportingFilters filters, CancellationToken ct = default);
    Task<ReportingDashboardDto> GetProcurementActivityAsync(ReportingFilters filters, CancellationToken ct = default);
    Task<ReportingDashboardDto> GetProcurementPlanningAsync(ReportingFilters filters, CancellationToken ct = default);
    Task<ReportingDashboardDto> GetRequisitionsAsync(ReportingFilters filters, CancellationToken ct = default);
    Task<ReportingDashboardDto> GetTendersAsync(ReportingFilters filters, CancellationToken ct = default);
    Task<ReportingDashboardDto> GetSupplierPerformanceAsync(ReportingFilters filters, CancellationToken ct = default);
    Task<ReportingDashboardDto> GetPurchaseOrdersAsync(ReportingFilters filters, CancellationToken ct = default);
    Task<ReportingDashboardDto> GetInvoicesAsync(ReportingFilters filters, CancellationToken ct = default);
    Task<ReportingDashboardDto> GetPoInvoiceAnalysisAsync(ReportingFilters filters, CancellationToken ct = default);
    Task<ReportingDashboardDto> GetComplianceAsync(ReportingFilters filters, CancellationToken ct = default);
    Task<ReportingDashboardDto> GetAuditAsync(ReportingFilters filters, CancellationToken ct = default);
    Task<ReportingDashboardDto> GetSpendAnalysisAsync(ReportingFilters filters, CancellationToken ct = default);
    Task<ReportingDashboardDto> GetWorkflowBottlenecksAsync(ReportingFilters filters, CancellationToken ct = default);
    Task<ReportingDashboardDto> GetManagementKpisAsync(ReportingFilters filters, CancellationToken ct = default);
    Task<ReportingDashboardDto> GetRfpEvidenceAsync(ReportingFilters filters, CancellationToken ct = default);
    Task<List<DataQualityIssueDto>> GetDataQualityAsync(ReportingFilters filters, CancellationToken ct = default);
    Task<List<ReportSubscription>> GetSubscriptionsAsync(CancellationToken ct = default);
    Task<ReportSubscription> CreateSubscriptionAsync(ReportSubscriptionDto dto, CancellationToken ct = default);
    Task<ReportSubscription?> UpdateSubscriptionAsync(Guid id, ReportSubscriptionDto dto, CancellationToken ct = default);
    Task<ReportRunHistory?> RunSubscriptionNowAsync(Guid id, CancellationToken ct = default);
    Task<string?> ExportCsvAsync(string reportCode, ReportingFilters filters, CancellationToken ct = default);
}

public sealed class ReportingApplicationService(EProcurementDbContext db) : IReportingApplicationService
{
    IQueryable<Requisition> Requisitions(ReportingFilters f) => db.Requisitions.AsNoTracking().Where(x => (!f.FinancialYearId.HasValue || x.FinancialYearId == f.FinancialYearId) && (string.IsNullOrWhiteSpace(f.Department) || x.Department == f.Department) && (!f.From.HasValue || x.CreatedAt >= f.From) && (!f.To.HasValue || x.CreatedAt <= f.To) && (string.IsNullOrWhiteSpace(f.Status) || x.Status.ToString() == f.Status));
    IQueryable<Tender> Tenders(ReportingFilters f) => db.Tenders.AsNoTracking().Where(x => (string.IsNullOrWhiteSpace(f.Category) || x.Category == f.Category) && (!f.From.HasValue || x.CreatedAt >= f.From) && (!f.To.HasValue || x.CreatedAt <= f.To) && (string.IsNullOrWhiteSpace(f.Status) || x.Status.ToString() == f.Status));
    IQueryable<PurchaseOrder> PurchaseOrders(ReportingFilters f) => db.PurchaseOrders.AsNoTracking().Where(x => (!f.SupplierId.HasValue || x.SupplierId == f.SupplierId) && (!f.From.HasValue || x.CreatedAt >= f.From) && (!f.To.HasValue || x.CreatedAt <= f.To) && (string.IsNullOrWhiteSpace(f.Status) || x.Status.ToString() == f.Status));
    IQueryable<AnnualProcurementPlan> Plans(ReportingFilters f) => db.AnnualProcurementPlans.AsNoTracking().Where(x => (!f.FinancialYearId.HasValue || x.FinancialYearId == f.FinancialYearId) && (string.IsNullOrWhiteSpace(f.Department) || x.Department == f.Department) && (!f.From.HasValue || x.CreatedAt >= f.From) && (!f.To.HasValue || x.CreatedAt <= f.To) && (string.IsNullOrWhiteSpace(f.Status) || x.Status == f.Status));
    static MetricDto M(string code, string label, decimal value, string unit = "count", string? drill = null) => new(code, label, value, unit, $"Counts {label.ToLowerInvariant()} from live application tables.", value == 0 ? "empty" : "ok", drill);
    static ReportingDashboardDto D(string code, string title, IEnumerable<MetricDto> metrics, IEnumerable<ReportRowDto>? rows = null, IEnumerable<string>? warnings = null) { var ms = metrics.ToList(); return new(code, title, ms, (rows ?? ms.Select(x => new ReportRowDto(title, x.Label, x.Value.ToString("0.##"), "Live procurement database"))).ToList(), DateTimeOffset.UtcNow, warnings?.ToList() ?? []); }
    public async Task<ReportingDashboardDto> GetExecutiveDashboardAsync(ReportingFilters f, CancellationToken ct = default)
    {
        var totalValue = (await Requisitions(f).SumAsync(x => (decimal?)x.EstimatedTotal, ct) ?? 0) + (await PurchaseOrders(f).SumAsync(x => (decimal?)x.TotalAmount, ct) ?? 0);
        var metrics = new[] { M("totalProcurementValue","Total procurement value",totalValue,"currency"), M("activeProcurementCases","Active procurement cases",await db.ProcurementCases.AsNoTracking().CountAsync(x=>x.Status==ProcurementCaseStatus.Active,ct)), M("publishedTenders","Published tenders",await Tenders(f).CountAsync(x=>x.Status==TenderStatus.Published,ct)), M("requisitionsAwaitingApproval","Requisitions awaiting approval",await Requisitions(f).CountAsync(x=>x.Status==RequisitionStatus.Submitted||x.Status==RequisitionStatus.ManagerApproval||x.Status==RequisitionStatus.ProcurementReview,ct)), M("evaluationsInProgress","Evaluations in progress",await db.EvaluationSessions.AsNoTracking().CountAsync(x=>x.Status==EvaluationSessionStatus.InProgress||x.Status==EvaluationSessionStatus.Consensus,ct)), M("awardsPendingApproval","Awards pending approval",await db.Awards.AsNoTracking().CountAsync(x=>x.Status==AwardStatus.UnderApproval||x.Status==AwardStatus.Recommended,ct)), M("purchaseOrdersIssued","Purchase orders issued",await PurchaseOrders(f).CountAsync(x=>x.Status==PurchaseOrderStatus.Issued||x.Status==PurchaseOrderStatus.Acknowledged||x.Status==PurchaseOrderStatus.PartiallyDelivered||x.Status==PurchaseOrderStatus.Delivered||x.Status==PurchaseOrderStatus.Closed,ct)), M("contractsActive","Contracts active",await db.Contracts.AsNoTracking().CountAsync(x=>x.Status==ContractStatus.Active,ct)), M("openSupplierRegistrations","Open supplier registrations",await db.Suppliers.AsNoTracking().CountAsync(x=>x.Status==SupplierStatus.Submitted||x.Status==SupplierStatus.UnderVerification,ct)), M("complianceExceptions","Compliance exceptions",await ComplianceExceptionCount(ct)) };
        return D("executive-dashboard", "Executive dashboard", metrics);
    }
    public async Task<ReportingDashboardDto> GetProcurementActivityAsync(ReportingFilters f, CancellationToken ct = default) => D("procurement-activity","Procurement activity", new[] { M("procurementCasesCreated","Procurement cases created",await db.ProcurementCases.AsNoTracking().CountAsync(ct), drill:"/app/procurement-cases"), M("requisitionsSubmitted","Requisitions submitted",await Requisitions(f).CountAsync(x=>x.Status!=RequisitionStatus.Draft,ct), drill:"/app/requisitions"), M("tendersCreated","Tenders created",await Tenders(f).CountAsync(ct), drill:"/app/tenders"), M("tendersPublished","Tenders published",await Tenders(f).CountAsync(x=>x.Status==TenderStatus.Published,ct), drill:"/app/tenders?status=Published"), M("bidsSubmitted","Bids submitted",await db.BidSubmissions.AsNoTracking().CountAsync(x=>x.Status!=BidSubmissionStatus.Draft,ct), drill:"/app/bids"), M("bidOpeningsCompleted","Bid openings completed",await db.BidOpeningSessions.AsNoTracking().CountAsync(x=>x.Status==BidOpeningSessionStatus.Completed,ct), drill:"/app/bid-opening"), M("evaluationsCompleted","Evaluations completed",await db.EvaluationSessions.AsNoTracking().CountAsync(x=>x.Status==EvaluationSessionStatus.Completed,ct), drill:"/app/evaluation"), M("awardsApproved","Awards approved",await db.Awards.AsNoTracking().CountAsync(x=>x.Status==AwardStatus.Approved||x.Status==AwardStatus.Published||x.Status==AwardStatus.ConvertedToPurchaseOrder||x.Status==AwardStatus.ConvertedToContract,ct), drill:"/app/awards"), M("purchaseOrdersIssued","Purchase orders issued",await PurchaseOrders(f).CountAsync(x=>x.Status!=PurchaseOrderStatus.Draft,ct), drill:"/app/purchase-orders"), M("contractsActivated","Contracts activated",await db.Contracts.AsNoTracking().CountAsync(x=>x.Status==ContractStatus.Active,ct), drill:"/app/contracts") });
    public async Task<ReportingDashboardDto> GetProcurementPlanningAsync(ReportingFilters f, CancellationToken ct = default) => D("procurement-planning","Procurement planning", new[] { M("annualPlans","Annual plans",await Plans(f).CountAsync(ct)), M("plannedValue","Planned value",await db.ProcurementPlanItems.AsNoTracking().SumAsync(x=>(decimal?)x.EstimatedAmount,ct)??0,"currency"), M("approvedPlanItems","Approved plan items",await db.ProcurementPlanItems.AsNoTracking().CountAsync(x=>x.Status=="Approved",ct)), M("plannedVsExecuted","Planned vs executed",await PurchaseOrders(f).SumAsync(x=>(decimal?)x.TotalAmount,ct)??0,"currency"), M("budgetUtilisation","Budget utilisation",await db.Budgets.AsNoTracking().SumAsync(x=>(decimal?)x.CommittedAmount,ct)??0,"currency") });
    public async Task<ReportingDashboardDto> GetRequisitionsAsync(ReportingFilters f, CancellationToken ct = default)
    {
        var approvalRows = await Requisitions(f).Where(x=>x.SubmittedAt!=null&&x.ApprovedAt!=null).Select(x=>new{x.SubmittedAt,x.ApprovedAt}).ToListAsync(ct);
        var avg = approvalRows.Count == 0 ? 0 : Math.Round((decimal)approvalRows.Average(x => (x.ApprovedAt!.Value - x.SubmittedAt!.Value).TotalDays), 2);
        return D("requisitions","Requisitions", new[] { M("totalRequisitions","Total requisitions",await Requisitions(f).CountAsync(ct)), M("draft","Draft",await Requisitions(f).CountAsync(x=>x.Status==RequisitionStatus.Draft,ct)), M("submitted","Submitted",await Requisitions(f).CountAsync(x=>x.Status==RequisitionStatus.Submitted,ct)), M("approved","Approved",await Requisitions(f).CountAsync(x=>x.Status==RequisitionStatus.Approved,ct)), M("rejected","Rejected",await Requisitions(f).CountAsync(x=>x.Status==RequisitionStatus.Rejected,ct)), M("averageApprovalTime","Average approval time", avg,"days"), M("budgetValidationFailures","Budget validation failures",await db.BusinessRuleExecutionLogs.AsNoTracking().CountAsync(x=>x.EntityType==nameof(Requisition)&&x.Outcome==RuleOutcome.Failed,ct)) });
    }
    public async Task<ReportingDashboardDto> GetTendersAsync(ReportingFilters f, CancellationToken ct = default) => D("tenders","Tenders", new[] { M("draftTenders","Draft tenders",await Tenders(f).CountAsync(x=>x.Status==TenderStatus.Draft,ct)), M("publishedTenders","Published tenders",await Tenders(f).CountAsync(x=>x.Status==TenderStatus.Published,ct)), M("closingSoon","Closing soon",await Tenders(f).CountAsync(x=>x.Status==TenderStatus.Published&&x.ClosingDate<=DateTimeOffset.UtcNow.AddDays(14)&&x.ClosingDate>=DateTimeOffset.UtcNow,ct)), M("closedTenders","Closed tenders",await Tenders(f).CountAsync(x=>x.Status==TenderStatus.Closed,ct)), M("tenderParticipationCount","Tender participation count",await db.BidSubmissions.AsNoTracking().CountAsync(ct)), M("clarificationsCount","Clarifications count",await db.TenderClarifications.AsNoTracking().CountAsync(ct)) });
    public async Task<ReportingDashboardDto> GetSupplierPerformanceAsync(ReportingFilters f, CancellationToken ct = default) => D("supplier-performance","Supplier performance", new[] { M("registeredSuppliers","Registered suppliers",await db.Suppliers.AsNoTracking().CountAsync(ct)), M("approvedSuppliers","Approved suppliers",await db.Suppliers.AsNoTracking().CountAsync(x=>x.Status==SupplierStatus.Approved,ct)), M("suspendedSuppliers","Suspended suppliers",await db.Suppliers.AsNoTracking().CountAsync(x=>x.Status==SupplierStatus.Suspended,ct)), M("blacklistedSuppliers","Blacklisted suppliers",await db.Suppliers.AsNoTracking().CountAsync(x=>x.Status==SupplierStatus.Blacklisted,ct)), M("supplierPerformanceRatings","Supplier performance ratings",await db.SupplierPerformanceRatings.AsNoTracking().CountAsync(ct)), M("documentsExpiringSoon","Documents expiring soon",0) });
    public async Task<ReportingDashboardDto> GetPurchaseOrdersAsync(ReportingFilters f, CancellationToken ct = default) => D("purchase-orders","Purchase orders", new[] { M("issuedPOs","Issued POs",await PurchaseOrders(f).CountAsync(x=>x.Status==PurchaseOrderStatus.Issued,ct)), M("acknowledgedPOs","Acknowledged POs",await PurchaseOrders(f).CountAsync(x=>x.Status==PurchaseOrderStatus.Acknowledged,ct)), M("partiallyDeliveredPOs","Partially delivered POs",await PurchaseOrders(f).CountAsync(x=>x.Status==PurchaseOrderStatus.PartiallyDelivered,ct)), M("closedPOs","Closed POs",await PurchaseOrders(f).CountAsync(x=>x.Status==PurchaseOrderStatus.Closed,ct)), M("cancelledPOs","Cancelled POs",await PurchaseOrders(f).CountAsync(x=>x.Status==PurchaseOrderStatus.Cancelled,ct)), M("poValue","PO value",await PurchaseOrders(f).SumAsync(x=>(decimal?)x.TotalAmount,ct)??0,"currency"), M("goodsReceipts","Goods receipts",await db.GoodsReceipts.AsNoTracking().CountAsync(ct)) });
    public async Task<ReportingDashboardDto> GetInvoicesAsync(ReportingFilters f, CancellationToken ct = default) => D("invoices","Invoices", new[] { M("totalInvoices","Total invoices",await db.SupplierInvoices.AsNoTracking().CountAsync(ct)), M("invoicesUnderReview","Invoices under review",await db.SupplierInvoices.AsNoTracking().CountAsync(x=>x.Status==SupplierInvoiceStatus.UnderReview||x.Status==SupplierInvoiceStatus.Submitted,ct)), M("matchedInvoices","Matched invoices",await db.SupplierInvoices.AsNoTracking().CountAsync(x=>x.Status==SupplierInvoiceStatus.Matched,ct)), M("invoicesWithExceptions","Invoices with exceptions",await db.SupplierInvoices.AsNoTracking().CountAsync(x=>x.Status==SupplierInvoiceStatus.Mismatch,ct)), M("approvedForPayment","Approved for payment",await db.SupplierInvoices.AsNoTracking().CountAsync(x=>x.Status==SupplierInvoiceStatus.ApprovedForPayment,ct)), M("rejectedInvoices","Rejected invoices",await db.SupplierInvoices.AsNoTracking().CountAsync(x=>x.Status==SupplierInvoiceStatus.Rejected,ct)), M("invoicedAmount","Invoiced amount",await db.SupplierInvoices.AsNoTracking().SumAsync(x=>(decimal?)x.TotalAmount,ct)??0,"currency") });
    public async Task<ReportingDashboardDto> GetPoInvoiceAnalysisAsync(ReportingFilters f, CancellationToken ct = default) => D("po-invoice-analysis","PO invoice analysis", new[] { M("poTotalAmount","PO total amount",await PurchaseOrders(f).SumAsync(x=>(decimal?)x.TotalAmount,ct)??0,"currency"), M("invoicedAmount","Invoiced amount",await db.SupplierInvoices.AsNoTracking().SumAsync(x=>(decimal?)x.TotalAmount,ct)??0,"currency"), M("receivedAmount","Received amount",await db.PurchaseOrderLines.AsNoTracking().SumAsync(x=>(decimal?)(x.DeliveredQuantity*x.UnitPrice),ct)??0,"currency"), M("outstandingInvoiceableAmount","Outstanding invoiceable amount",(await PurchaseOrders(f).SumAsync(x=>(decimal?)x.TotalAmount,ct)??0)-(await db.SupplierInvoices.AsNoTracking().SumAsync(x=>(decimal?)x.TotalAmount,ct)??0),"currency"), M("varianceAmount","Variance amount",await db.ThreeWayMatches.AsNoTracking().SumAsync(x=>(decimal?)x.VarianceAmount,ct)??0,"currency"), M("returnsValue","Returns value",await db.PurchaseOrderReturnLines.AsNoTracking().SumAsync(x=>(decimal?)x.TotalAmount,ct)??0,"currency"), M("creditNotesValue","Credit notes value",0,"currency") });
    public async Task<ReportingDashboardDto> GetComplianceAsync(ReportingFilters f, CancellationToken ct = default) => D("compliance","Compliance", new[] { M("auditExceptions","Audit exceptions",await ComplianceExceptionCount(ct)), M("failedPermissionAttempts","Failed permission attempts",await db.AuditEvents.AsNoTracking().CountAsync(x=>x.EventType.Contains("Permission")&&x.Details.Contains("Failed"),ct)), M("lateBids","Late bids",await db.BidOpeningSubmissions.AsNoTracking().CountAsync(x=>x.Status==BidOpeningSubmissionStatus.Late,ct)), M("missingDocuments","Missing documents",await db.AuditEvents.AsNoTracking().CountAsync(x=>x.Details.Contains("Missing document"),ct)), M("approvalDelays","Approval delays",await db.WorkflowTasks.AsNoTracking().CountAsync(x=>x.Status!=WorkflowTaskStatus.Completed&&x.CreatedAt<DateTimeOffset.UtcNow.AddDays(-7),ct)), M("workflowBottlenecks","Workflow bottlenecks",await db.WorkflowTasks.AsNoTracking().CountAsync(x=>x.Status==WorkflowTaskStatus.Open&&x.CreatedAt<DateTimeOffset.UtcNow.AddDays(-3),ct)) });
    public async Task<ReportingDashboardDto> GetAuditAsync(ReportingFilters f, CancellationToken ct = default)
    {
        var groups = await db.AuditEvents.AsNoTracking().GroupBy(x=>x.EntityType).Select(g=>new MetricDto(g.Key, "Audit events - "+g.Key, g.Count())).ToListAsync(ct);
        return D("audit","Audit", groups.Count == 0 ? new[] { M("auditEvents","Audit events",0) } : groups);
    }
    async Task<decimal> ComplianceExceptionCount(CancellationToken ct) => await db.AuditEvents.AsNoTracking().CountAsync(x=>x.EventType.Contains("Exception")||x.EventType.Contains("Failed")||x.Details.Contains("Exception")||x.Details.Contains("Missing")||x.Details.Contains("Late"),ct);
    public async Task<ReportingDashboardDto> GetSpendAnalysisAsync(ReportingFilters f, CancellationToken ct = default)
    {
        var planned = await db.ProcurementPlanItems.AsNoTracking().SumAsync(x => (decimal?)x.EstimatedAmount, ct) ?? 0;
        var awarded = await db.Awards.AsNoTracking().SumAsync(x => (decimal?)x.AwardAmount, ct) ?? 0;
        var po = await PurchaseOrders(f).SumAsync(x => (decimal?)x.TotalAmount, ct) ?? 0;
        var contracts = await db.Contracts.AsNoTracking().SumAsync(x => (decimal?)x.CurrentValue, ct) ?? 0;
        return D("spend-analysis", "Spend analysis", new[] { M("totalPlannedSpend","Total planned spend",planned,"currency"), M("totalAwardedSpend","Total awarded spend",awarded,"currency"), M("totalPoSpend","Total PO spend",po,"currency"), M("totalContractValue","Total contract value",contracts,"currency"), M("spendVariance","Spend variance",planned-po,"currency"), M("spendUnderActiveContracts","Spend under active contracts",await db.Contracts.AsNoTracking().Where(x=>x.Status==ContractStatus.Active).SumAsync(x=>(decimal?)x.CurrentValue,ct)??0,"currency") });
    }
    public async Task<ReportingDashboardDto> GetWorkflowBottlenecksAsync(ReportingFilters f, CancellationToken ct = default)
    {
        var completed = await db.WorkflowTasks.AsNoTracking().Where(x=>x.Status==WorkflowTaskStatus.Completed).Select(x=>new{x.CreatedAt,x.CompletedAt}).ToListAsync(ct);
        var avg = completed.Count == 0 ? 0 : Math.Round((decimal)completed.Where(x=>x.CompletedAt!=null).DefaultIfEmpty().Average(x=>x==null||x.CompletedAt==null?0:(x.CompletedAt.Value-x.CreatedAt).TotalDays),2);
        return D("workflow-bottlenecks", "Workflow bottlenecks", new[] { M("openWorkflowTasks","Open workflow tasks",await db.WorkflowTasks.AsNoTracking().CountAsync(x=>x.Status==WorkflowTaskStatus.Open||x.Status==WorkflowTaskStatus.Assigned,ct), drill:"/app/tasks"), M("overdueWorkflowTasks","Overdue workflow tasks",await db.WorkflowTasks.AsNoTracking().CountAsync(x=>x.Status!=WorkflowTaskStatus.Completed&&x.CreatedAt<DateTimeOffset.UtcNow.AddDays(-7),ct), drill:"/app/tasks?status=overdue"), M("averageTaskCompletionTime","Average task completion time",avg,"days"), M("escalationsTriggered","Escalations triggered",await db.WorkflowTaskEscalations.AsNoTracking().CountAsync(ct)), M("delegatedTasks","Delegated tasks",await db.DelegationRules.AsNoTracking().CountAsync(x=>x.IsActive,ct)) });
    }
    public async Task<ReportingDashboardDto> GetManagementKpisAsync(ReportingFilters f, CancellationToken ct = default)
    {
        var defs = SeedKpiDefinitions();
        var tenders = await Tenders(f).CountAsync(ct); var bids = await db.BidSubmissions.AsNoTracking().CountAsync(ct); var reqs = await Requisitions(f).CountAsync(ct);
        var budgetTotal = await db.Budgets.AsNoTracking().SumAsync(x=>(decimal?)x.TotalAmount,ct)??0; var committed = await db.Budgets.AsNoTracking().SumAsync(x=>(decimal?)x.CommittedAmount,ct)??0;
        decimal Val(string code) => code switch { "AVERAGE_BIDS_PER_TENDER" => tenders==0?0:Math.Round((decimal)bids/tenders,2), "BUDGET_UTILISATION_RATE" => budgetTotal==0?0:Math.Round(committed/budgetTotal*100,2), "COMPLIANCE_EXCEPTION_RATE" => reqs==0?0:Math.Round(ComplianceExceptionCount(ct).GetAwaiter().GetResult()/Math.Max(1,reqs)*100,2), _ => 0 };
        var metrics = defs.Select(d => M(d.Code, d.Name, Val(d.Code), d.Unit)).ToList();
        var rows = defs.Select(d => new ReportRowDto(d.Category, d.Name, Val(d.Code).ToString("0.##"), d.Description + " Source: live procurement tables")).ToList();
        return D("management-kpis","Management KPI dashboard",metrics,rows);
    }
    public Task<ReportingDashboardDto> GetRfpEvidenceAsync(ReportingFilters f, CancellationToken ct = default)
    {
        var rows = SeedEvidence().Where(x => string.IsNullOrWhiteSpace(f.Status) || x.Status.ToString() == f.Status).Select(x => new ReportRowDto(x.RequirementArea, x.RequirementText, x.Status.ToString(), $"{x.Module} | {x.Route} | {x.ApiEndpoint} | {x.EvidenceNotes}"));
        var all = SeedEvidence().Where(x => string.IsNullOrWhiteSpace(f.Status) || x.Status.ToString() == f.Status).ToList();
        return Task.FromResult(D("rfp-evidence","RFP evidence center", Enum.GetNames<RequirementEvidenceStatus>().Select(st => M(st, st, all.Count(x=>x.Status.ToString()==st))), rows));
    }
    public async Task<List<DataQualityIssueDto>> GetDataQualityAsync(ReportingFilters f, CancellationToken ct = default)
    {
        var issues = new List<DataQualityIssueDto>();
        issues.AddRange(await db.Tenders.AsNoTracking().Where(x=>x.Category=="" || x.Category==null!).Select(x=>new DataQualityIssueDto("TENDER_NO_CATEGORY","Warning","Tender",x.TenderNumber,"Tender has no category",$"/app/tenders/{x.Id}")).ToListAsync(ct));
        var publishedIds = await db.PublicTenderPublications.AsNoTracking().Select(x=>x.TenderId).ToListAsync(ct);
        issues.AddRange(await db.Tenders.AsNoTracking().Where(x=>x.Status==TenderStatus.Published&&!publishedIds.Contains(x.Id)).Select(x=>new DataQualityIssueDto("PUBLISHED_TENDER_NO_PUBLICATION","High","Tender",x.TenderNumber,"Published tender has no public publication",$"/app/tenders/{x.Id}")).ToListAsync(ct));
        issues.AddRange(await db.Requisitions.AsNoTracking().Where(x=>!db.BudgetCommitments.Any(b=>b.RequisitionId==x.Id)).Select(x=>new DataQualityIssueDto("REQUISITION_NO_BUDGET","Warning","Requisition",x.RequisitionNumber,"Requisition has no budget commitment",$"/app/requisitions/{x.Id}")).ToListAsync(ct));
        issues.AddRange(await db.AuditEvents.AsNoTracking().Where(x=>x.Actor=="").Select(x=>new DataQualityIssueDto("AUDIT_MISSING_ACTOR","Warning","AuditEvent",x.EntityReference,"Audit event has missing actor","/app/reporting/audit")).ToListAsync(ct));
        return issues;
    }
    public Task<List<ReportSubscription>> GetSubscriptionsAsync(CancellationToken ct = default) => db.ReportSubscriptions.AsNoTracking().ToListAsync(ct);
    public async Task<ReportSubscription> CreateSubscriptionAsync(ReportSubscriptionDto dto, CancellationToken ct = default) { var s = new ReportSubscription(dto.ReportCode,dto.UserId,dto.Frequency,dto.Format,dto.FiltersJson,dto.IsActive,DateTimeOffset.UtcNow); db.ReportSubscriptions.Add(s); db.AuditEvents.Add(new AuditEvent("Report subscription created","ReportSubscription",s.Id,s.ReportCode,dto.UserId,"Report subscription created",DateTimeOffset.UtcNow)); await db.SaveChangesAsync(ct); return s; }
    public async Task<ReportSubscription?> UpdateSubscriptionAsync(Guid id, ReportSubscriptionDto dto, CancellationToken ct = default) { var s = await db.ReportSubscriptions.FindAsync([id],ct); if (s is null) return null; db.Entry(s).CurrentValues.SetValues(s with { ReportCode = dto.ReportCode, UserId = dto.UserId, Frequency = dto.Frequency, Format = dto.Format, FiltersJson = dto.FiltersJson, IsActive = dto.IsActive }); await db.SaveChangesAsync(ct); return s; }
    public async Task<ReportRunHistory?> RunSubscriptionNowAsync(Guid id, CancellationToken ct = default) { var s = await db.ReportSubscriptions.AsNoTracking().SingleOrDefaultAsync(x=>x.Id==id,ct); if (s is null) return null; var run = new ReportRunHistory(s.ReportCode,s.UserId,DateTimeOffset.UtcNow,DateTimeOffset.UtcNow,"Completed",$"{s.ReportCode}.csv",null); db.ReportRunHistories.Add(run); db.AuditEvents.Add(new AuditEvent("Report subscription run","ReportSubscription",id,s.ReportCode,s.UserId,"Report subscription run",DateTimeOffset.UtcNow)); await db.SaveChangesAsync(ct); return run; }
    static List<KpiDefinition> SeedKpiDefinitions() => new() { new("PROCUREMENT_CYCLE_TIME","Procurement cycle time","Average elapsed days from requisition to contract or PO.","Cycle Time","AverageDays",30,"days","Active"), new("REQUISITION_APPROVAL_TURNAROUND","Requisition approval turnaround time","Average days from submission to approval.","Cycle Time","AverageDays",5,"days","Active"), new("TENDER_PUBLICATION_TURNAROUND","Tender publication turnaround time","Average days from tender draft to publication.","Tendering","AverageDays",10,"days","Active"), new("AVERAGE_BIDS_PER_TENDER","Average bids per tender","Submitted bid count divided by tender count.","Participation","Ratio",3,"count","Active"), new("SUPPLIER_PARTICIPATION_RATE","Supplier participation rate","Suppliers submitting bids divided by registered suppliers.","Participation","Percent",50,"percent","Active"), new("EVALUATION_COMPLETION_TIME","Evaluation completion time","Average elapsed days for completed evaluations.","Evaluation","AverageDays",14,"days","Active"), new("AWARD_APPROVAL_TURNAROUND","Award approval turnaround time","Average award approval days.","Awards","AverageDays",7,"days","Active"), new("PO_ISSUE_TURNAROUND","PO issue turnaround time","Average days from award approval to PO issue.","Purchase Orders","AverageDays",5,"days","Active"), new("CONTRACT_ACTIVATION_TIME","Contract activation time","Average days from award to active contract.","Contracts","AverageDays",10,"days","Active"), new("COMPLIANCE_EXCEPTION_RATE","Compliance exception rate","Compliance exceptions per requisition percentage.","Compliance","Percent",5,"percent","Active"), new("SUPPLIER_DOCUMENT_COMPLIANCE_RATE","Supplier document compliance rate","Suppliers with required documents percentage.","Supplier","Percent",95,"percent","Active"), new("BUDGET_UTILISATION_RATE","Budget utilisation rate","Committed budget divided by total budget.","Budget","Percent",90,"percent","Active") };
    static List<RequirementEvidenceItem> SeedEvidence() { string[] areas = ["Supplier Management","Tender Management","Clarifications","Sealed Bid Submission","Bid Opening","Evaluation","Contracts Integration","Requisitions","Budget Validation","Purchase Orders","Annual Planning","Reporting","Security","Compliance","Integration","Document Management","Notifications","Configurable Workflows","Business Rules","Data Governance","Migration","Support and Maintenance"]; return areas.Select(a => new RequirementEvidenceItem(a.ToUpperInvariant().Replace(" ","_"), RequirementEvidenceSource.RFP, a, $"RFP capability for {a}", $"ProcuraFlow provides phase 1 coverage for {a}", a, a.Contains("Reporting")?"ReportingEngine":a, a switch { "Reporting" => "/app/reporting", "Supplier Management" => "/app/suppliers", "Tender Management" => "/app/tenders", "Requisitions" => "/app/requisitions", "Purchase Orders" => "/app/purchase-orders", "Annual Planning" => "/app/planning", _ => "/app/reporting/rfp-evidence" }, a=="Reporting"?"/api/reporting/executive-dashboard":"", a is "Migration" or "Support and Maintenance" ? RequirementEvidenceStatus.NeedsConfiguration : a is "Contracts Integration" or "Data Governance" ? RequirementEvidenceStatus.PartiallyCovered : RequirementEvidenceStatus.Covered, "Evidence seeded honestly from implemented routes, APIs and configured modules.", DateTimeOffset.UtcNow)).ToList(); }
    public async Task<string?> ExportCsvAsync(string reportCode, ReportingFilters f, CancellationToken ct = default)
    {
        var report = reportCode switch { "executive-dashboard" => await GetExecutiveDashboardAsync(f,ct), "procurement-activity" => await GetProcurementActivityAsync(f,ct), "procurement-planning" => await GetProcurementPlanningAsync(f,ct), "requisitions" => await GetRequisitionsAsync(f,ct), "tenders" => await GetTendersAsync(f,ct), "supplier-performance" => await GetSupplierPerformanceAsync(f,ct), "purchase-orders" => await GetPurchaseOrdersAsync(f,ct), "invoices" => await GetInvoicesAsync(f,ct), "po-invoice-analysis" => await GetPoInvoiceAnalysisAsync(f,ct), "spend-analysis" => await GetSpendAnalysisAsync(f,ct), "compliance" => await GetComplianceAsync(f,ct), "audit" => await GetAuditAsync(f,ct), "workflow-bottlenecks" => await GetWorkflowBottlenecksAsync(f,ct), "management-kpis" => await GetManagementKpisAsync(f,ct), "rfp-evidence" => await GetRfpEvidenceAsync(f,ct), _ => null };
        if (report is null) return null;
        db.AuditEvents.Add(new AuditEvent("Report exported","Report",Guid.Empty,reportCode,"system",$"Report exported with filters {JsonSerializer.Serialize(f)}",DateTimeOffset.UtcNow)); await db.SaveChangesAsync(ct);
        return string.Join("\n", new[] { "Area,Metric,Value,Source", $"Filters,{Csv(JsonSerializer.Serialize(f))},," }.Concat(report.Rows.Select(r => $"{Csv(r.Area)},{Csv(r.Metric)},{Csv(r.Value)},{Csv(r.Source)}")));
    }
    static string Csv(string value) => $"\"{value.Replace("\"", "\"\"")}\"";
}

public interface ISealedBidAccessService
{
...
public interface ISealedBidApplicationService : ISealedBidAccessService
{
    Task<SealedBidEnvelope?> GetEnvelopeAsync(Guid bidSubmissionId, CancellationToken ct = default);
    Task<BidSubmissionReceiptDto?> GetReceiptAsync(Guid bidSubmissionId, CancellationToken ct = default);
    Task<List<BidAccessLog>> GetAccessLogAsync(Guid bidSubmissionId, CancellationToken ct = default);
    Task<List<BidOpeningEvidence>> GetOpeningEvidenceAsync(Guid sessionId, CancellationToken ct = default);
    Task<BidOpeningEvidence?> GetOpeningEvidenceAsync(Guid sessionId, Guid bidSubmissionId, CancellationToken ct = default);
    Task<IntegrityCheckDto?> CheckIntegrityAsync(Guid bidSubmissionId, string actor, Guid? sessionId = null, CancellationToken ct = default);
...
public sealed class SealedBidApplicationService(EProcurementDbContext db) : ISealedBidApplicationService
{
    const string HashAlgorithm = "SHA256";
    public Task<SealedBidEnvelope?> GetEnvelopeAsync(Guid bidSubmissionId, CancellationToken ct = default) => db.SealedBidEnvelopes.AsNoTracking().SingleOrDefaultAsync(x => x.BidSubmissionId == bidSubmissionId, ct);
    public Task<List<BidAccessLog>> GetAccessLogAsync(Guid bidSubmissionId, CancellationToken ct = default) => db.BidAccessLogs.AsNoTracking().Where(x => x.BidSubmissionId == bidSubmissionId).OrderByDescending(x => x.AccessedAt).ToListAsync(ct);
    public Task<List<BidOpeningEvidence>> GetOpeningEvidenceAsync(Guid sessionId, CancellationToken ct = default) => db.BidOpeningEvidence.AsNoTracking().Where(x => x.BidOpeningSessionId == sessionId).OrderByDescending(x => x.OpenedAt).ToListAsync(ct);
    public Task<BidOpeningEvidence?> GetOpeningEvidenceAsync(Guid sessionId, Guid bidSubmissionId, CancellationToken ct = default) => db.BidOpeningEvidence.AsNoTracking().SingleOrDefaultAsync(x => x.BidOpeningSessionId == sessionId && x.BidSubmissionId == bidSubmissionId, ct);
    public async Task<SealedBidDocumentEvidence> RecordDocumentEvidenceAsync(BidSubmission bid, BidSubmissionDocument doc, CancellationToken ct = default)
    {
        var hash = Sha($"{doc.Id}|{doc.BidSubmissionId}|{doc.DocumentType}|{doc.Filename}|{doc.StorageReference}|{doc.UploadedAt:O}|{doc.Version}");
        var ev = new SealedBidDocumentEvidence(doc.Id, bid.Id, doc.Filename, doc.DocumentType, doc.StorageReference, System.Text.Encoding.UTF8.GetByteCount(doc.StorageReference), hash, HashAlgorithm, doc.UploadedAt, null, false, DateTimeOffset.UtcNow);
        db.SealedBidDocumentEvidence.Add(ev);
        db.AuditEvents.Add(new AuditEvent("Bid document hash calculated", nameof(BidSubmission), bid.Id, bid.SubmissionNumber, doc.UploadedBy, hash, DateTimeOffset.UtcNow));
        return ev;
    }
    public async Task<SealedBidEnvelope> SealAsync(BidSubmission bid, string actor, CancellationToken ct = default)
    {
        var now = DateTimeOffset.UtcNow;
        var manifest = await ManifestHash(bid.Id, ct);
        var submissionHash = SubmissionHash(bid, manifest);
        var envelope = new SealedBidEnvelope(bid.Id, bid.TenderId, bid.SupplierId, $"ENV-{now:yyyyMMddHHmmss}-{bid.SubmissionNumber}", SealedBidEnvelopeStatus.Sealed, now, actor, null, null, null, submissionHash, manifest, $"system-time:{now:O}", null, $"vault://bid-submissions/{bid.Id}", null, now);
        db.SealedBidEnvelopes.Add(envelope);
        await db.SealedBidDocumentEvidence.Where(x => x.BidSubmissionId == bid.Id).ExecuteUpdateAsync(s => s.SetProperty(x => x.SealedAt, now), ct);
        db.AuditEvents.Add(new AuditEvent("Bid sealed", nameof(BidSubmission), bid.Id, bid.SubmissionNumber, actor, envelope.EnvelopeNumber, now));
        db.AuditEvents.Add(new AuditEvent("Bid submission receipt generated", nameof(BidSubmission), bid.Id, bid.SubmissionNumber, actor, submissionHash, now));
        return envelope;
    }
    public async Task<BidSubmissionReceiptDto?> GetReceiptAsync(Guid bidSubmissionId, CancellationToken ct = default)
    {
        var bid = await db.BidSubmissions.AsNoTracking().Include(x=>x.Declarations).SingleOrDefaultAsync(x=>x.Id==bidSubmissionId, ct); if (bid is null) return null;
        var env = await db.SealedBidEnvelopes.AsNoTracking().SingleOrDefaultAsync(x=>x.BidSubmissionId==bidSubmissionId, ct); if (env is null) return null;
        var tender = await db.Tenders.AsNoTracking().SingleAsync(x=>x.Id==bid.TenderId, ct); var supplier = await db.Suppliers.AsNoTracking().SingleAsync(x=>x.Id==bid.SupplierId, ct);
        var docs = await db.SealedBidDocumentEvidence.AsNoTracking().Where(x=>x.BidSubmissionId==bidSubmissionId).OrderBy(x=>x.DocumentType).ToListAsync(ct);
        return new(bid.SubmissionNumber, tender.TenderNumber, tender.Title, supplier.LegalName, bid.SubmittedBy, bid.SubmittedAt, env.EnvelopeNumber, env.SubmissionHash, env.DocumentManifestHash, docs, bid.Declarations, DateTimeOffset.UtcNow, $"RCPT-{env.EnvelopeNumber}");
    }
    public async Task<IntegrityCheckDto?> CheckIntegrityAsync(Guid bidSubmissionId, string actor, Guid? sessionId = null, CancellationToken ct = default)
    {
        var bid = await db.BidSubmissions.AsNoTracking().Include(x=>x.Items).Include(x=>x.Declarations).SingleOrDefaultAsync(x=>x.Id==bidSubmissionId, ct); if (bid is null) return null;
        var env = await db.SealedBidEnvelopes.AsNoTracking().SingleOrDefaultAsync(x=>x.BidSubmissionId==bidSubmissionId, ct); if (env is null) return null;
        var manifest = await ManifestHash(bid.Id, ct); var subHash = SubmissionHash(bid, manifest); var passed = manifest == env.DocumentManifestHash && subHash == env.SubmissionHash;
        var json = JsonSerializer.Serialize(new { expectedSubmissionHash = env.SubmissionHash, actualSubmissionHash = subHash, expectedDocumentManifestHash = env.DocumentManifestHash, actualDocumentManifestHash = manifest, passed });
        db.AuditEvents.Add(new AuditEvent(passed ? "Bid integrity check passed" : "Bid integrity check failed", nameof(BidSubmission), bid.Id, bid.SubmissionNumber, actor, json, DateTimeOffset.UtcNow));
        await LogAccessAttempt(bid.Id, actor, BidAccessType.IntegrityCheck, true, null, ct: ct);
        return new(bid.Id, subHash, manifest, passed, json);
    }
    public async Task<BidOpeningEvidence> RecordOpeningAsync(BidOpeningSession session, BidSubmission bid, string actor, string reason, CancellationToken ct = default)
    {
        var check = await CheckIntegrityAsync(bid.Id, actor, session.Id, ct) ?? throw new InvalidOperationException("Missing sealed envelope."); var now = DateTimeOffset.UtcNow;
        var evidence = new BidOpeningEvidence(session.Id, bid.Id, actor, now, reason, check.SubmissionHash, check.DocumentManifestHash, check.Passed, check.ResultJson, now);
        db.BidOpeningEvidence.Add(evidence);
        var env = await db.SealedBidEnvelopes.SingleAsync(x => x.BidSubmissionId == bid.Id, ct);
        db.Entry(env).CurrentValues[nameof(SealedBidEnvelope.Status)] = SealedBidEnvelopeStatus.Opened; db.Entry(env).CurrentValues[nameof(SealedBidEnvelope.OpenedAt)] = now; db.Entry(env).CurrentValues[nameof(SealedBidEnvelope.OpenedBy)] = actor; db.Entry(env).CurrentValues[nameof(SealedBidEnvelope.OpeningSessionId)] = session.Id;
        db.AuditEvents.Add(new AuditEvent("Sealed bid opened", nameof(BidOpeningSession), session.Id, session.SessionNumber, actor, check.ResultJson, now));
        return evidence;
    }
    public async Task<bool> CanViewBidPricing(string userEmail, Guid bidSubmissionId, CancellationToken ct = default) { var opened = await db.SealedBidEnvelopes.AnyAsync(x=>x.BidSubmissionId==bidSubmissionId && x.Status==SealedBidEnvelopeStatus.Opened, ct); await LogAccessAttempt(bidSubmissionId, userEmail, BidAccessType.ViewPricing, opened, opened?null:"Pricing access denied before opening", ct: ct); return opened; }
    public async Task<bool> CanViewBidDocument(string userEmail, Guid bidSubmissionId, Guid documentId, CancellationToken ct = default) { var opened = await db.SealedBidEnvelopes.AnyAsync(x=>x.BidSubmissionId==bidSubmissionId && x.Status==SealedBidEnvelopeStatus.Opened, ct); await LogAccessAttempt(bidSubmissionId, userEmail, BidAccessType.ViewDocument, opened, opened?null:"Sealed bid document access denied", ct: ct); return opened; }
    public Task LogAccessAttempt(Guid bidSubmissionId, string userEmail, BidAccessType accessType, bool allowed, string? deniedReason, string? ip = null, string? userAgent = null, CancellationToken ct = default) { db.BidAccessLogs.Add(new BidAccessLog(bidSubmissionId, userEmail, userEmail, accessType, allowed, deniedReason, DateTimeOffset.UtcNow, ip, userAgent)); db.AuditEvents.Add(new AuditEvent(allowed ? "Bid access allowed" : "Bid access denied", nameof(BidSubmission), bidSubmissionId, bidSubmissionId.ToString(), userEmail, deniedReason ?? accessType.ToString(), DateTimeOffset.UtcNow)); return Task.CompletedTask; }
    async Task<string> ManifestHash(Guid bidId, CancellationToken ct) { var hashes = await db.SealedBidDocumentEvidence.AsNoTracking().Where(x=>x.BidSubmissionId==bidId).OrderBy(x=>x.DocumentType).ThenBy(x=>x.FileName).Select(x=>x.ContentHash).ToListAsync(ct); return Sha(string.Join('|', hashes)); }
    static string SubmissionHash(BidSubmission bid, string manifest) => Sha(JsonSerializer.Serialize(new { bid.SubmissionNumber, bid.TenderId, bid.SupplierId, totals = bid.Items.Sum(x=>x.Total), manifest, declarations = bid.Declarations.OrderBy(x=>x.DeclarationType).Select(x=>new{x.DeclarationType,x.Accepted}) }));
...
public interface IPurchaseOrderReturnApplicationService
{
    Task<List<PurchaseOrderReturn>> GetAsync(CancellationToken ct = default); Task<PurchaseOrderReturn?> GetAsync(Guid id, CancellationToken ct = default);
    Task<PurchaseOrderReturn> CreateAsync(CreatePurchaseOrderReturnDto dto, CancellationToken ct = default); Task<PurchaseOrderReturn?> SubmitAsync(Guid id, ActorDto dto, CancellationToken ct = default); Task<PurchaseOrderReturn?> ApproveAsync(Guid id, ActorDto dto, CancellationToken ct = default); Task<PurchaseOrderReturn?> RejectAsync(Guid id, ActorDto dto, CancellationToken ct = default); Task<PurchaseOrderReturn?> CompleteAsync(Guid id, ActorDto dto, CancellationToken ct = default); Task<PurchaseOrderReturn?> CancelAsync(Guid id, ActorDto dto, CancellationToken ct = default);
}
public interface ISupplierInvoiceApplicationService
{
    Task<List<SupplierInvoice>> GetAsync(Guid? supplierId = null, CancellationToken ct = default); Task<SupplierInvoice?> GetAsync(Guid id, CancellationToken ct = default);
    Task<SupplierInvoice> CreateAsync(CreateSupplierInvoiceDto dto, CancellationToken ct = default); Task<SupplierInvoice> CreateFromPurchaseOrderAsync(Guid purchaseOrderId, ActorDto dto, CancellationToken ct = default);
    Task<SupplierInvoice?> SubmitAsync(Guid id, ActorDto dto, CancellationToken ct = default); Task<SupplierInvoice?> RunMatchAsync(Guid id, ActorDto dto, CancellationToken ct = default); Task<SupplierInvoice?> ApproveForPaymentAsync(Guid id, ActorDto dto, CancellationToken ct = default); Task<SupplierInvoice?> RejectAsync(Guid id, RejectInvoiceDto dto, CancellationToken ct = default); Task<SupplierInvoice?> CancelAsync(Guid id, ActorDto dto, CancellationToken ct = default);
    Task<List<InvoiceStatusHistory>> GetHistoryAsync(Guid id, CancellationToken ct = default); Task<List<InvoiceAttachment>> GetAttachmentsAsync(Guid id, CancellationToken ct = default); Task<InvoiceAttachment> AddAttachmentAsync(Guid id, InvoiceAttachmentDto dto, CancellationToken ct = default);
}
public interface IInvoiceMatchingApplicationService { Task<List<ThreeWayMatch>> GetAsync(CancellationToken ct = default); Task<ThreeWayMatch?> GetAsync(Guid invoiceId, CancellationToken ct = default); Task<ThreeWayMatch> RunAsync(Guid invoiceId, ActorDto dto, CancellationToken ct = default); Task<List<InvoiceMatchingResult>> GetResultsAsync(Guid invoiceId, CancellationToken ct = default); }
public sealed record ActorDto(string Actor, string? Notes = null);
public sealed record CreatePurchaseOrderReturnDto(Guid PurchaseOrderId, string Reason, string CreatedBy, string? Notes, List<CreatePurchaseOrderReturnLineDto> Lines);
public sealed record CreatePurchaseOrderReturnLineDto(Guid PurchaseOrderLineId, decimal ReturnedQuantity, string Reason, string? ConditionNotes = null);
public sealed record CreateSupplierInvoiceDto(string SupplierInvoiceNumber, Guid PurchaseOrderId, Guid? GoodsReceiptId, DateTimeOffset InvoiceDate, DateTimeOffset? ReceivedDate, string Currency, string SubmittedBy, string? Notes, List<CreateSupplierInvoiceLineDto> Lines);
public sealed record CreateSupplierInvoiceLineDto(Guid? PurchaseOrderLineId, Guid? GoodsReceiptId, int ItemNumber, string Description, decimal Quantity, decimal UnitPrice, decimal TaxAmount);
public sealed record InvoiceAttachmentDto(string DocumentType, string FileName, string StorageReference, string UploadedBy);
public sealed record RejectInvoiceDto(string Actor, string RejectionReason, string? Notes = null);

public sealed class PurchaseOrderReturnApplicationService(EProcurementDbContext db) : IPurchaseOrderReturnApplicationService
{
    public Task<List<PurchaseOrderReturn>> GetAsync(CancellationToken ct = default) => db.PurchaseOrderReturns.AsNoTracking().Include(x=>x.Lines).OrderByDescending(x=>x.CreatedAt).ToListAsync(ct);
    public Task<PurchaseOrderReturn?> GetAsync(Guid id, CancellationToken ct = default) => db.PurchaseOrderReturns.AsNoTracking().Include(x=>x.Lines).SingleOrDefaultAsync(x=>x.Id==id,ct);
    public async Task<PurchaseOrderReturn> CreateAsync(CreatePurchaseOrderReturnDto dto, CancellationToken ct = default){var po=await db.PurchaseOrders.Include(x=>x.Lines).SingleAsync(x=>x.Id==dto.PurchaseOrderId,ct); var now=DateTimeOffset.UtcNow; var r=new PurchaseOrderReturn($"POR-{now:yyyyMMddHHmmssfff}",po.Id,po.SupplierId,po.SupplierName,dto.Reason,PurchaseOrderReturnStatus.Draft,dto.CreatedBy,now,Notes:dto.Notes); foreach(var l in dto.Lines){var pl=po.Lines.Single(x=>x.Id==l.PurchaseOrderLineId); var approvedReturned=await db.PurchaseOrderReturnLines.Where(x=>x.PurchaseOrderLineId==pl.Id).Join(db.PurchaseOrderReturns.Where(x=>x.Status==PurchaseOrderReturnStatus.Approved||x.Status==PurchaseOrderReturnStatus.Completed),x=>x.PurchaseOrderReturnId,x=>x.Id,(line,ret)=>line.ReturnedQuantity).SumAsync(ct); if(l.ReturnedQuantity+approvedReturned>pl.DeliveredQuantity) throw new InvalidOperationException("Returned quantity cannot exceed delivered quantity."); r.Lines.Add(new(r.Id,pl.Id,pl.ItemNumber,pl.Description,l.ReturnedQuantity,pl.UnitPrice,l.ReturnedQuantity*pl.UnitPrice,l.Reason,l.ConditionNotes));} db.PurchaseOrderReturns.Add(r); Audit("PO return created",nameof(PurchaseOrderReturn),r.Id,r.ReturnNumber,dto.CreatedBy,dto.Reason); await db.SaveChangesAsync(ct); return r;}
    public Task<PurchaseOrderReturn?> SubmitAsync(Guid id, ActorDto dto, CancellationToken ct = default)=>Move(id,PurchaseOrderReturnStatus.Submitted,dto,"PO return submitted",ct);
    public async Task<PurchaseOrderReturn?> ApproveAsync(Guid id, ActorDto dto, CancellationToken ct = default){var r=await db.PurchaseOrderReturns.Include(x=>x.Lines).SingleOrDefaultAsync(x=>x.Id==id,ct); if(r is null)return null; foreach(var l in r.Lines){var pl=await db.PurchaseOrderLines.SingleAsync(x=>x.Id==l.PurchaseOrderLineId,ct); db.Entry(pl).CurrentValues[nameof(PurchaseOrderLine.DeliveredQuantity)]=Math.Max(0,pl.DeliveredQuantity-l.ReturnedQuantity); db.Entry(pl).CurrentValues[nameof(PurchaseOrderLine.OutstandingQuantity)]=pl.OutstandingQuantity+l.ReturnedQuantity;} db.Entry(r).CurrentValues[nameof(PurchaseOrderReturn.ApprovedBy)]=dto.Actor; db.Entry(r).CurrentValues[nameof(PurchaseOrderReturn.ApprovedAt)]=DateTimeOffset.UtcNow; await Set(r,PurchaseOrderReturnStatus.Approved,dto,"PO return approved",ct); return r;}
    public Task<PurchaseOrderReturn?> RejectAsync(Guid id, ActorDto dto, CancellationToken ct = default)=>Move(id,PurchaseOrderReturnStatus.Rejected,dto,"PO return rejected",ct); public Task<PurchaseOrderReturn?> CompleteAsync(Guid id, ActorDto dto, CancellationToken ct = default)=>Move(id,PurchaseOrderReturnStatus.Completed,dto,"PO return completed",ct); public Task<PurchaseOrderReturn?> CancelAsync(Guid id, ActorDto dto, CancellationToken ct = default)=>Move(id,PurchaseOrderReturnStatus.Cancelled,dto,"PO return cancelled",ct);
    async Task<PurchaseOrderReturn?> Move(Guid id,PurchaseOrderReturnStatus st,ActorDto dto,string evt,CancellationToken ct){var r=await db.PurchaseOrderReturns.Include(x=>x.Lines).SingleOrDefaultAsync(x=>x.Id==id,ct); if(r is null)return null; await Set(r,st,dto,evt,ct); return r;} async Task Set(PurchaseOrderReturn r,PurchaseOrderReturnStatus st,ActorDto dto,string evt,CancellationToken ct){db.Entry(r).CurrentValues[nameof(PurchaseOrderReturn.Status)]=st; if(st==PurchaseOrderReturnStatus.Submitted)db.Entry(r).CurrentValues[nameof(PurchaseOrderReturn.SubmittedAt)]=DateTimeOffset.UtcNow; if(st==PurchaseOrderReturnStatus.Completed)db.Entry(r).CurrentValues[nameof(PurchaseOrderReturn.CompletedAt)]=DateTimeOffset.UtcNow; if(st==PurchaseOrderReturnStatus.Cancelled)db.Entry(r).CurrentValues[nameof(PurchaseOrderReturn.CancelledAt)]=DateTimeOffset.UtcNow; Audit(evt,nameof(PurchaseOrderReturn),r.Id,r.ReturnNumber,dto.Actor,dto.Notes??evt); await db.SaveChangesAsync(ct);} void Audit(string e,string t,Guid id,string r,string a,string d)=>db.AuditEvents.Add(new AuditEvent(e,t,id,r,a,d,DateTimeOffset.UtcNow));
}

public sealed class InvoiceMatchingApplicationService(EProcurementDbContext db) : IInvoiceMatchingApplicationService
{
    public Task<List<ThreeWayMatch>> GetAsync(CancellationToken ct = default)=>db.ThreeWayMatches.AsNoTracking().Include(x=>x.Results).OrderByDescending(x=>x.CreatedAt).ToListAsync(ct); public Task<ThreeWayMatch?> GetAsync(Guid invoiceId,CancellationToken ct=default)=>db.ThreeWayMatches.AsNoTracking().Include(x=>x.Results).Where(x=>x.SupplierInvoiceId==invoiceId).OrderByDescending(x=>x.CreatedAt).FirstOrDefaultAsync(ct); public async Task<List<InvoiceMatchingResult>> GetResultsAsync(Guid invoiceId,CancellationToken ct=default){var m=await GetAsync(invoiceId,ct); return m?.Results.OrderBy(x=>x.Message).ToList()??[];}
    public async Task<ThreeWayMatch> RunAsync(Guid invoiceId, ActorDto dto, CancellationToken ct = default){var inv=await db.SupplierInvoices.Include(x=>x.Lines).SingleAsync(x=>x.Id==invoiceId,ct); var po=await db.PurchaseOrders.Include(x=>x.Lines).SingleAsync(x=>x.Id==inv.PurchaseOrderId,ct); db.AuditEvents.Add(new AuditEvent("Invoice matching started",nameof(SupplierInvoice),inv.Id,inv.InvoiceNumber,dto.Actor,"Three way match started",DateTimeOffset.UtcNow)); var receiptAmount=po.Lines.Sum(x=>x.DeliveredQuantity*x.UnitPrice); var match=new ThreeWayMatch(inv.Id,po.Id,inv.GoodsReceiptId,InvoiceMatchStatus.Matched,po.TotalAmount,receiptAmount,inv.TotalAmount,po.TotalAmount-inv.TotalAmount,po.TotalAmount==0?0:Math.Round((po.TotalAmount-inv.TotalAmount)/po.TotalAmount*100,2),true,true,inv.GoodsReceiptId is not null||po.GoodsReceipts.Any(),true,"{}",DateTimeOffset.UtcNow,dto.Actor); bool ok=po.Status!=PurchaseOrderStatus.Cancelled && inv.SupplierId==po.SupplierId; foreach(var l in inv.Lines){var pl=l.PurchaseOrderLineId.HasValue?po.Lines.FirstOrDefault(x=>x.Id==l.PurchaseOrderLineId):po.Lines.FirstOrDefault(x=>x.ItemNumber==l.ItemNumber); var returned=pl is null?0:await db.PurchaseOrderReturnLines.Where(x=>x.PurchaseOrderLineId==pl.Id).Join(db.PurchaseOrderReturns.Where(x=>x.Status==PurchaseOrderReturnStatus.Approved||x.Status==PurchaseOrderReturnStatus.Completed),x=>x.PurchaseOrderReturnId,x=>x.Id,(line,ret)=>line.ReturnedQuantity).SumAsync(ct); var received=Math.Max(0,(pl?.DeliveredQuantity??0)-returned); var msg=pl is null?"Invoice line does not exist on PO": l.Quantity>received?"Invoice quantity exceeds received quantity or returned quantity impact": l.UnitPrice!=(pl?.UnitPrice??0)?"Invoice unit price variance": l.TotalAmount!=l.Quantity*l.UnitPrice+l.TaxAmount?"Invoice total calculation mismatch":"Matched"; var st=msg=="Matched"?InvoiceMatchStatus.Matched:InvoiceMatchStatus.Exception; ok&=st==InvoiceMatchStatus.Matched; match.Results.Add(new(match.Id,l.Id,pl?.Id,st,pl?.Quantity??0,l.Quantity,received,pl?.UnitPrice??0,l.UnitPrice,(pl is null?0:(pl.UnitPrice-l.UnitPrice)*l.Quantity),msg));} if(!ok){match=match with{MatchStatus=InvoiceMatchStatus.Exception,QuantityMatched=false,PriceMatched=match.Results.All(x=>x.ExpectedUnitPrice==x.InvoicedUnitPrice)};} db.ThreeWayMatches.Add(match); db.Entry(inv).CurrentValues[nameof(SupplierInvoice.Status)]=ok?SupplierInvoiceStatus.Matched:SupplierInvoiceStatus.Mismatch; db.InvoiceStatusHistories.Add(new(inv.Id,inv.Status,ok?SupplierInvoiceStatus.Matched:SupplierInvoiceStatus.Mismatch,dto.Actor,DateTimeOffset.UtcNow,"Invoice matching completed")); db.AuditEvents.Add(new AuditEvent(ok?"Invoice matching completed":"Invoice matching exception",nameof(SupplierInvoice),inv.Id,inv.InvoiceNumber,dto.Actor,ok?"Matched":"Exception",DateTimeOffset.UtcNow)); await db.SaveChangesAsync(ct); return match;}
}

public sealed class SupplierInvoiceApplicationService(EProcurementDbContext db, IInvoiceMatchingApplicationService matching) : ISupplierInvoiceApplicationService
{
    public Task<List<SupplierInvoice>> GetAsync(Guid? supplierId=null,CancellationToken ct=default)=>db.SupplierInvoices.AsNoTracking().Include(x=>x.Lines).Where(x=>!supplierId.HasValue||x.SupplierId==supplierId).OrderByDescending(x=>x.ReceivedDate).ToListAsync(ct); public Task<SupplierInvoice?> GetAsync(Guid id,CancellationToken ct=default)=>db.SupplierInvoices.AsNoTracking().Include(x=>x.Lines).Include(x=>x.Attachments).SingleOrDefaultAsync(x=>x.Id==id,ct);
    public async Task<SupplierInvoice> CreateFromPurchaseOrderAsync(Guid purchaseOrderId, ActorDto dto, CancellationToken ct=default){var po=await db.PurchaseOrders.Include(x=>x.Lines).SingleAsync(x=>x.Id==purchaseOrderId,ct); var lines=po.Lines.Select(x=>new CreateSupplierInvoiceLineDto(x.Id,null,x.ItemNumber,x.Description,x.DeliveredQuantity==0?x.Quantity:x.DeliveredQuantity,x.UnitPrice,0)).ToList(); return await CreateAsync(new($"SUP-{DateTimeOffset.UtcNow:yyyyMMddHHmmss}",po.Id,null,DateTimeOffset.UtcNow,DateTimeOffset.UtcNow,po.Currency,dto.Actor,dto.Notes,lines),ct);}
    public async Task<SupplierInvoice> CreateAsync(CreateSupplierInvoiceDto dto,CancellationToken ct=default){var po=await db.PurchaseOrders.SingleAsync(x=>x.Id==dto.PurchaseOrderId,ct); if(await db.SupplierInvoices.AnyAsync(x=>x.SupplierId==po.SupplierId&&x.SupplierInvoiceNumber==dto.SupplierInvoiceNumber,ct)) throw new InvalidOperationException("Duplicate supplier invoice number for same supplier."); var now=DateTimeOffset.UtcNow; var inv=new SupplierInvoice($"INV-{now:yyyyMMddHHmmssfff}",dto.SupplierInvoiceNumber,po.Id,dto.GoodsReceiptId,po.SupplierId,po.SupplierName,dto.InvoiceDate,dto.ReceivedDate??now,dto.Currency,0,0,0,SupplierInvoiceStatus.Draft,dto.SubmittedBy,Notes:dto.Notes); foreach(var l in dto.Lines) inv.Lines.Add(new(inv.Id,l.PurchaseOrderLineId,l.GoodsReceiptId,l.ItemNumber,l.Description,l.Quantity,l.UnitPrice,l.TaxAmount,l.Quantity*l.UnitPrice+l.TaxAmount)); var sub=inv.Lines.Sum(x=>x.Quantity*x.UnitPrice); db.SupplierInvoices.Add(inv); db.Entry(inv).CurrentValues[nameof(SupplierInvoice.SubtotalAmount)]=sub; db.Entry(inv).CurrentValues[nameof(SupplierInvoice.TaxAmount)]=inv.Lines.Sum(x=>x.TaxAmount); db.Entry(inv).CurrentValues[nameof(SupplierInvoice.TotalAmount)]=inv.Lines.Sum(x=>x.TotalAmount); Audit("Invoice created",inv,dto.SubmittedBy,dto.SupplierInvoiceNumber); await db.SaveChangesAsync(ct); return inv;}
    public async Task<SupplierInvoice?> SubmitAsync(Guid id,ActorDto dto,CancellationToken ct=default){var i=await db.SupplierInvoices.SingleOrDefaultAsync(x=>x.Id==id,ct); if(i is null)return null; var po=await db.PurchaseOrders.SingleAsync(x=>x.Id==i.PurchaseOrderId,ct); if(po.Status==PurchaseOrderStatus.Cancelled) throw new InvalidOperationException("Invoice cannot be submitted for cancelled PO."); return await Move(i,SupplierInvoiceStatus.Submitted,dto,"Invoice submitted",ct);} public async Task<SupplierInvoice?> RunMatchAsync(Guid id,ActorDto dto,CancellationToken ct=default){await matching.RunAsync(id,dto,ct); return await GetAsync(id,ct);} public async Task<SupplierInvoice?> ApproveForPaymentAsync(Guid id,ActorDto dto,CancellationToken ct=default){var i=await db.SupplierInvoices.SingleOrDefaultAsync(x=>x.Id==id,ct); if(i is null)return null; if(i.Status==SupplierInvoiceStatus.Mismatch&&!await db.InvoiceReviews.AnyAsync(x=>x.SupplierInvoiceId==id,ct)) throw new InvalidOperationException("Invoice with mismatch requires review before approval."); await Move(i,SupplierInvoiceStatus.ApprovedForPayment,dto,"Invoice approved for payment",ct); var prep=new PaymentPreparation(i.Id,$"PAYPREP-{DateTimeOffset.UtcNow:yyyyMMddHHmmssfff}",PaymentPreparationStatus.Pending,dto.Actor,DateTimeOffset.UtcNow,ErrorMessage:"Finance endpoint not configured."); db.PaymentPreparations.Add(prep); db.AuditEvents.Add(new AuditEvent("Payment preparation created",nameof(SupplierInvoice),i.Id,i.InvoiceNumber,dto.Actor,"Finance endpoint not configured.",DateTimeOffset.UtcNow)); await db.SaveChangesAsync(ct); return i;} public async Task<SupplierInvoice?> RejectAsync(Guid id,RejectInvoiceDto dto,CancellationToken ct=default){var i=await db.SupplierInvoices.SingleOrDefaultAsync(x=>x.Id==id,ct); if(i is null)return null; db.Entry(i).CurrentValues[nameof(SupplierInvoice.RejectionReason)]=dto.RejectionReason; return await Move(i,SupplierInvoiceStatus.Rejected,new(dto.Actor,dto.Notes),"Invoice rejected",ct);} public async Task<SupplierInvoice?> CancelAsync(Guid id,ActorDto dto,CancellationToken ct=default){var i=await db.SupplierInvoices.SingleOrDefaultAsync(x=>x.Id==id,ct); return i is null?null:await Move(i,SupplierInvoiceStatus.Cancelled,dto,"Invoice cancelled",ct);} public Task<List<InvoiceStatusHistory>> GetHistoryAsync(Guid id,CancellationToken ct=default)=>db.InvoiceStatusHistories.AsNoTracking().Where(x=>x.SupplierInvoiceId==id).OrderBy(x=>x.ChangedAt).ToListAsync(ct); public Task<List<InvoiceAttachment>> GetAttachmentsAsync(Guid id,CancellationToken ct=default)=>db.InvoiceAttachments.AsNoTracking().Where(x=>x.SupplierInvoiceId==id).ToListAsync(ct); public async Task<InvoiceAttachment> AddAttachmentAsync(Guid id,InvoiceAttachmentDto dto,CancellationToken ct=default){var a=new InvoiceAttachment(id,dto.DocumentType,dto.FileName,dto.StorageReference,dto.UploadedBy,DateTimeOffset.UtcNow); db.InvoiceAttachments.Add(a); db.AuditEvents.Add(new AuditEvent("Invoice attachment uploaded",nameof(SupplierInvoice),id,dto.FileName,dto.UploadedBy,dto.DocumentType,DateTimeOffset.UtcNow)); await db.SaveChangesAsync(ct); return a;} async Task<SupplierInvoice> Move(SupplierInvoice i,SupplierInvoiceStatus to,ActorDto dto,string evt,CancellationToken ct){var from=i.Status; db.Entry(i).CurrentValues[nameof(SupplierInvoice.Status)]=to; if(to==SupplierInvoiceStatus.Submitted)db.Entry(i).CurrentValues[nameof(SupplierInvoice.SubmittedAt)]=DateTimeOffset.UtcNow; if(to==SupplierInvoiceStatus.ApprovedForPayment)db.Entry(i).CurrentValues[nameof(SupplierInvoice.ApprovedForPaymentAt)]=DateTimeOffset.UtcNow; if(to==SupplierInvoiceStatus.Rejected)db.Entry(i).CurrentValues[nameof(SupplierInvoice.RejectedAt)]=DateTimeOffset.UtcNow; db.InvoiceStatusHistories.Add(new(i.Id,from,to,dto.Actor,DateTimeOffset.UtcNow,dto.Notes??evt)); Audit(evt,i,dto.Actor,dto.Notes??evt); await db.SaveChangesAsync(ct); return i;} void Audit(string e,SupplierInvoice i,string a,string d)=>db.AuditEvents.Add(new AuditEvent(e,nameof(SupplierInvoice),i.Id,i.InvoiceNumber,a,d,DateTimeOffset.UtcNow));
}
