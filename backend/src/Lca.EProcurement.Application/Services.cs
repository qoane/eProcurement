...
public sealed record ReportingFilters(Guid? FinancialYearId = null, string? Department = null, string? Category = null, Guid? SupplierId = null, DateTimeOffset? From = null, DateTimeOffset? To = null, string? Status = null);
public sealed record MetricDto(string Code, string Label, decimal Value, string Unit = "count");
public sealed record ReportingDashboardDto(string Code, string Title, List<MetricDto> Metrics, List<ReportRowDto> Rows);
...
public sealed class ReportingApplicationService(EProcurementDbContext db) : IReportingApplicationService
{
    ...
    static MetricDto M(string code, string label, decimal value, string unit = "count") => new(code, label, value, unit);
    static ReportingDashboardDto D(string code, string title, IEnumerable<MetricDto> metrics) { var ms = metrics.ToList(); return new(code, title, ms, ms.Select(x => new ReportRowDto(title, x.Label, x.Value.ToString("0.##"), "Live procurement database")).ToList()); }
    ...
    {
        var report = reportCode switch { "executive-dashboard" => await GetExecutiveDashboardAsync(f,ct), "procurement-planning" => await GetProcurementPlanningAsync(f,ct), "requisitions" => await GetRequisitionsAsync(f,ct), "tenders" => await GetTendersAsync(f,ct), "supplier-performance" => await GetSupplierPerformanceAsync(f,ct), "purchase-orders" => await GetPurchaseOrdersAsync(f,ct), "invoices" => await GetInvoicesAsync(f,ct), "po-invoice-analysis" => await GetPoInvoiceAnalysisAsync(f,ct), "compliance" => await GetComplianceAsync(f,ct), "audit" => await GetAuditAsync(f,ct), _ => null };
        if (report is null) return null;
        return string.Join("\n", new[] { "Area,Metric,Value,Source" }.Concat(report.Rows.Select(r => $"{Csv(r.Area)},{Csv(r.Metric)},{Csv(r.Value)},{Csv(r.Source)}")));
    }
