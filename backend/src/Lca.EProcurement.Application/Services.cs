...
public sealed class ConfigurationStudioApplicationService(EProcurementDbContext db) : IConfigurationStudioApplicationService
{
    ...
    public async Task<object> PreviewAsync(string kind, JsonElement payload, CancellationToken ct = default) => kind.ToLowerInvariant() switch { "approval-matrix" => await PreviewApproval(payload,ct), "document-requirements" => await PreviewDocs(payload,ct), "workflow" => (object?)await db.WorkflowDefinitions.AsNoTracking().Include(x=>x.Versions).ThenInclude(v=>v.Nodes).Include(x=>x.Versions).ThenInclude(v=>v.Transitions).FirstOrDefaultAsync(x=>x.Code==payload.GetProperty("code").GetString(),ct) ?? new { error="Workflow not found" }, "form" => (object?)await db.FormDefinitions.AsNoTracking().Include(x=>x.Versions).ThenInclude(v=>v.Sections).ThenInclude(s=>s.Fields).FirstOrDefaultAsync(x=>x.Code==payload.GetProperty("code").GetString(),ct) ?? new { error="Form not found" }, _ => new { kind, preview=true, payload } };
