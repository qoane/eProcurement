...
public interface IBusinessRuleApplicationService { Task<RuleResult> EvaluateAsync(string ruleCode, string entityType, Guid entityId, string actor, CancellationToken ct = default); }
...
public interface IDynamicFormApplicationService { Task<FormDefinition> CreateDefinitionAsync(CreateFormDefinitionDto dto, CancellationToken ct = default); Task<FormDefinition?> PublishVersionAsync(string code, string actor, CancellationToken ct = default); Task<FormDefinition?> GetActiveByCodeAsync(string code, CancellationToken ct = default); Task<FormSubmission> SubmitAsync(SubmitFormDto dto, CancellationToken ct = default); Task<List<FormSubmission>> GetSubmissionsAsync(string entityType, Guid entityId, CancellationToken ct = default); }
