...
public record PageDefinition(string Code, string Name, string Description, PageType PageType, string DatasourceJson = "{}", string LayoutJson = "{}", string ToolbarJson = "[]", string ActionsJson = "[]", string FiltersJson = "[]", string ColumnsJson = "[]", string ComponentsJson = "[]", string PermissionsJson = "[]", string NavigationJson = "{}", int Version = 1, MetadataStatus Status = MetadataStatus.Draft, DateTimeOffset Created = default, DateTimeOffset? Modified = null, string CreatedBy = "system", string? ModifiedBy = null) : MetadataEntity(Guid.NewGuid(), Code, Name, Description, Version, Status, Created == default ? DateTimeOffset.UtcNow : Created, Modified, CreatedBy, ModifiedBy);
