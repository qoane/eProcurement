...
public static class SeedData
{
    ...
    private static async Task SeedIdentityAsync(EProcurementDbContext db, CancellationToken ct)
    {
        ...
        foreach (var (code, name, cat) in new[] { ("Reporting.View","View reporting","Reports"),("Reporting.Export","Export reporting","Reports"),("Reporting.Executive","View executive dashboard","Reports"),("Reporting.ProcurementActivity","View procurement activity reports","Reports"),("Reporting.Planning","View planning reports","Reports"),("Reporting.Requisitions","View requisition reports","Reports"),("Reporting.Tenders","View tender reports","Reports"),("Reporting.SupplierPerformance","View supplier performance reports","Reports"),("Reporting.PurchaseOrders","View purchase order reports","Reports"),("Reporting.SpendAnalysis","View spend analysis reports","Reports"),("Reporting.Compliance","View compliance reports","Reports"),("Reporting.Audit","View audit reports","Reports"),("Reporting.WorkflowBottlenecks","View workflow bottleneck reports","Reports"),("Reporting.ManagementKpis","View management KPI reports","Reports"),("Reporting.RfpEvidence","View RFP evidence","Reports"),("Reporting.Subscriptions.Manage","Manage report subscriptions","Reports"),("Operations.View","View operations","Operations"),("Operations.Manage","Manage operations","Operations"),("Operations.Health.View","View operations health","Operations"),("Operations.Performance.View","View performance","Operations"),("Operations.Backup.View","View backups","Operations"),("Operations.Backup.Manage","Manage backups","Operations"),("Operations.Configuration.Validate","Validate configuration","Operations"),("SupportCase.View","View support cases","Support"),("SupportCase.Create","Create support cases","Support"),("SupportCase.Manage","Manage support cases","Support") })
            if (!await db.Permissions.AnyAsync(x => x.Code == code, ct)) db.Permissions.Add(new Permission(code, name, name, cat));
        await db.SaveChangesAsync(ct);
        ...
        var rolePerms = new Dictionary<string,string[]> {
            ["System Administrator"] = all.Keys.ToArray(), ["Procurement Officer"] = ["Supplier.View","Supplier.Create","Supplier.Verify","Planning.View","Planning.Create","Requisition.View","Requisition.Create","Tender.View","Tender.Create","Tender.Publish","BidOpening.View","BidOpening.Create","Evaluation.View","Award.View","PurchaseOrder.View","Invoice.View","InvoiceMatching.View","PurchaseOrderReturn.View","PurchaseOrderReturn.Create","Contract.View","Audit.View","Integrations.View","Integrations.Manage","Migration.View","Migration.Upload","Migration.Validate","DataQuality.View","DataExport.Request","DataExport.Download","SupportCase.Create","SupportCase.View","Reporting.View","Reporting.ProcurementActivity","Reporting.Planning","Reporting.Requisitions","Reporting.Tenders","Reporting.SupplierPerformance","Reporting.PurchaseOrders","Reporting.SpendAnalysis","Reporting.ManagementKpis"],
            ["Finance User"] = ["Budget.View","Budget.Create","Planning.View","Planning.Approve","Requisition.View","Requisition.Approve","PurchaseOrder.View","Report.View","Invoice.View","Invoice.Review","Invoice.ApproveForPayment","Invoice.Reject","InvoiceMatching.View","InvoiceMatching.Run","PurchaseOrderReturn.View","Reporting.InvoiceAnalysis","Reporting.View","Reporting.PurchaseOrders","Reporting.SpendAnalysis","Reporting.ManagementKpis","DataQuality.View","DataExport.Request","DataExport.Download","SupportCase.Create","SupportCase.View"], ["Approver"] = ["Planning.Approve","Requisition.Approve","Award.Approve","PurchaseOrder.View","Contract.View","Report.View"],
            ["Evaluator"] = ["Reporting.View","Reporting.Tenders","Tender.View","BidOpening.View","Evaluation.View","Evaluation.Score","Evaluation.Consensus","Evaluation.Recommend"], ["Auditor"] = ["Dashboard.View","Supplier.View","Planning.View","Budget.View","Requisition.View","Tender.View","Bid.View","BidOpening.View","Evaluation.View","Award.View","PurchaseOrder.View","Invoice.View","InvoiceMatching.View","PurchaseOrderReturn.View","PurchaseOrderReturn.Create","Contract.View","Audit.View","Integrations.View","Integrations.Manage","Report.View","Reporting.View","Reporting.Compliance","Reporting.Audit","Reporting.RfpEvidence","Reporting.Export","Notifications.View","NotificationLogs.View","DataGovernance.View","DataRetention.View","DataArchive.View","Migration.View","DataQuality.View","DataExport.View","DataExport.Download","PrivacyClassification.View","Operations.View","Operations.Health.View","Operations.Performance.View","SupportCase.View"],
            ["Supplier"] = ["Supplier.View","Supplier.Create","Bid.View","Bid.Create","Bid.Submit","Bid.Withdraw","Tender.View","Invoice.View","Invoice.Create","Invoice.Submit","Notifications.View","SupportCase.Create","SupportCase.View"] };
    ...
    private static async Task SeedRfpEvidenceAsync(EProcurementDbContext db, CancellationToken ct)
    {
        var evidencePerms = new[] { "RfpEvidence.View", "RfpEvidence.Manage", "RfpEvidence.Export", "Demo.View", "Demo.Manage", "Uat.View", "Uat.Manage", "Uat.Execute", "Training.View", "Training.Manage", "Implementation.View", "Implementation.Manage", "Handover.View", "Handover.Manage", "Readiness.View", "Readiness.Manage", "SupportMaintenance.View", "SupportMaintenance.Manage" };
        foreach (var code in evidencePerms) if (!await db.Permissions.AnyAsync(x => x.Code == code, ct)) db.Permissions.Add(new Permission(code, code, code, "RFP Evidence"));
        await db.SaveChangesAsync(ct);
        var allPerms = await db.Permissions.ToDictionaryAsync(x => x.Code, ct); var roles = await db.Roles.ToDictionaryAsync(x => x.Name, ct);
        var map = new Dictionary<string,string[]> { ["System Administrator"] = evidencePerms, ["Procurement Officer"] = ["RfpEvidence.View","Demo.View","Uat.View","Uat.Execute","Training.View","Implementation.View","Handover.View","Readiness.View"], ["Auditor"] = ["RfpEvidence.View","RfpEvidence.Export","Uat.View","Readiness.View","Handover.View"], ["Supplier"] = ["Training.View"], ["Management"] = ["RfpEvidence.View","Demo.View","Readiness.View","Implementation.View","SupportMaintenance.View"] };
        if (!roles.ContainsKey("Management")) { db.Roles.Add(new Role("Management", "Management")); await db.SaveChangesAsync(ct); roles = await db.Roles.ToDictionaryAsync(x => x.Name, ct); }
        foreach (var (role, perms) in map) if (roles.TryGetValue(role, out var r)) foreach (var perm in perms) if (allPerms.TryGetValue(perm, out var pp) && !await db.RolePermissions.AnyAsync(x=>x.RoleId==r.Id && x.PermissionId==pp.Id, ct)) db.RolePermissions.Add(new RolePermission(r.Id, pp.Id));
        await db.SaveChangesAsync(ct);
        var now = DateTimeOffset.UtcNow;
        (string area,string[] reqs,ComplianceRequirementType type)[] groups = [
            ("Supplier Management", ["Self service supplier registration","Supplier verification","Supplier categorisation","Supplier profiling","Supplier performance management","Suspension and blacklisting","Supplier document repository"], ComplianceRequirementType.Functional),
            ("Sourcing and Tender Management", ["RFI management","RFQ management","RFP management","Tender publishing","Supplier notifications","Sealed bid submission","Controlled bid opening","Technical evaluation","Financial evaluation","Configurable scoring","Clarifications","Tender committees","Approval workflows"], ComplianceRequirementType.Functional),
            ("Contract Management Integration", ["Integration with existing LCA contract system"], ComplianceRequirementType.Integration),
            ("Requisition and Purchase Order Management", ["Requisition creation","Requisition approvals","Budget validation","Automated PO generation","PO amendments","PO cancellations","PO returns"], ComplianceRequirementType.Functional),
            ("Annual Planning", ["Create annual procurement plans","Upload annual procurement plans"], ComplianceRequirementType.Functional),
            ("Reporting and Analytics", ["PO invoice analysis","Procurement dashboard","Requisition dashboard","Compliance reporting","Audit reporting","Supplier performance dashboard"], ComplianceRequirementType.Reporting),
            ("Security", ["Multi level approval hierarchies","Delegation","Escalation","Enterprise identity","SSO","MFA","Encryption","Secure bid submission","Non repudiation","Audit trails","RBAC","Least privilege"], ComplianceRequirementType.Security),
            ("Compliance and Governance", ["Policy enforcement","Threshold enforcement","Workflow controls"], ComplianceRequirementType.Governance),
            ("Integration", ["Secure interfaces","Future systems readiness","Document management integration","Email integration","Automated notifications"], ComplianceRequirementType.Integration),
            ("Technical Architecture", ["Configurable workflows","Business rules","Scalable modular architecture","Concurrent users","Performance","Intuitive UI","Data ownership","Privacy","Retention","Archival","Historic data migration"], ComplianceRequirementType.Technical),
            ("Implementation", ["Architecture diagrams","Integration approach","Methodology","Timelines","Training","Support","Maintenance"], ComplianceRequirementType.Implementation) ];
        var n=1; foreach (var g in groups) foreach (var text in g.reqs) { var code=$"RFP-{n:000}"; if (!await db.ComplianceRequirements.AnyAsync(x=>x.RequirementCode==code, ct)) db.ComplianceRequirements.Add(new ComplianceRequirement(code, ComplianceSource.RFP, g.area, g.area, text, g.type, g.type==ComplianceRequirementType.Security?"Critical":"High", text.Contains("Future")||text.Contains("Historic")?ComplianceStatus.NeedsConfiguration:ComplianceStatus.Covered, g.area, RouteFor(g.area), ApiFor(g.area), $"Evidence mapped for {text}; verify during UAT.", "Procurement PMO", now, "system", now, null)); n++; }
        string[] commitments = ["End to end procurement automation","Annual procurement planning","Supplier onboarding","Sourcing","Tender management","Evaluation","Contract integration","Purchase order management","Reporting","Audit oversight","Configurable workflows","Supplier portal","Bid submission","Clarification response","Dashboards","Security controls","SSO readiness","MFA","RBAC","Encryption","Audit logging","Integration capability","Support and maintenance","Training and knowledge transfer"];
        n=1; foreach (var c in commitments) { var code=$"PROP-{n:000}"; if (!await db.ProposalCommitments.AnyAsync(x=>x.CommitmentCode==code, ct)) db.ProposalCommitments.Add(new ProposalCommitment(code,"Technical Proposal",c,c,c.Contains("Training")?"Training":c.Contains("Support")?"Support":"ProcuraFlow",RouteFor(c),ApiFor(c), c.Contains("readiness")?ComplianceStatus.NeedsConfiguration:ComplianceStatus.Covered,$"Proposal commitment evidence for {c}.",now)); n++; }
        string[] steps = ["Open executive dashboard","Show annual procurement plan","Show approved requisition and budget validation","Publish tender","Open public portal","Show supplier opportunity","Submit supplier bid","Show sealed bid protection","Run bid opening","Run evaluation","Approve award","Generate purchase order","Activate contract","Show reports","Show audit trail","Show configuration studio"];
        for (int i=0;i<steps.Length;i++) if (!await db.DemoSteps.AnyAsync(x=>x.StepNumber==i+1, ct)) db.DemoSteps.Add(new DemoStep(i+1, steps[i], "LCA ICT equipment end-to-end demo step.", i<6?"Procurement Officer":i<8?"Supplier":"Evaluation/Approver", DemoRoute(i+1), DemoRef(i+1), "Connect this step to RFP compliance, auditability and governance.", "Presenter can show traceable procurement evidence.", "Ready", now, null));
        string[] suites=["Supplier Management UAT","Tender Management UAT","Bid Submission UAT","Bid Opening UAT","Evaluation UAT","Award and PO UAT","Contract Integration UAT","Reporting UAT","Security UAT","Supplier Portal UAT","Public Portal UAT","Document Management UAT","Notifications UAT"];
        foreach (var s in suites) if (!await db.UatTestSuites.AnyAsync(x=>x.Name==s, ct)) { var suite=new UatTestSuite(s.ToUpperInvariant().Replace(" ","-").Replace("&","AND"),s,$"Acceptance scenarios for {s}.",s.Replace(" UAT",""),now,"system"); suite.TestCases.Add(new UatTestCase(suite.Id,"TC-001",$"Validate {s}","Demo data is seeded.","Open module; execute core scenario; capture evidence.","Scenario completes with audit trail.","RFP-001","High","Ready")); db.UatTestSuites.Add(suite); }
        (string code,string name,TrainingAudience aud,string route)[] mods=[("INTRO","Introduction to ProcuraFlow",TrainingAudience.Management,"/app/dashboard"),("SUP-REG","Supplier Registration and Verification",TrainingAudience.ProcurementOfficer,"/app/suppliers"),("APP","Annual Procurement Planning",TrainingAudience.ProcurementOfficer,"/app/planning"),("REQ","Requisition and Budget Validation",TrainingAudience.ProcurementOfficer,"/app/requisitions"),("TENDER","Tender Publication",TrainingAudience.ProcurementOfficer,"/app/tenders"),("SUP-BID","Supplier Bid Submission",TrainingAudience.Supplier,"/supplier/bids"),("BID-OPEN","Bid Opening",TrainingAudience.ProcurementOfficer,"/app/bid-openings"),("EVAL","Evaluation and Scoring",TrainingAudience.Evaluator,"/app/evaluations"),("AWARD","Award Management",TrainingAudience.Approver,"/app/awards"),("PO","Purchase Order Management",TrainingAudience.FinanceUser,"/app/purchase-orders"),("CONTRACT","Contract Management",TrainingAudience.ProcurementOfficer,"/app/contracts"),("REPORT","Reporting and Dashboards",TrainingAudience.Management,"/app/reporting"),("AUDIT","Audit and Compliance",TrainingAudience.Auditor,"/app/audit"),("STUDIO","Configuration Studio",TrainingAudience.SystemAdministrator,"/app/studio"),("SEC","Security Administration",TrainingAudience.SystemAdministrator,"/app/security"),("SUPPLIER-GUIDE","Supplier Portal Guide",TrainingAudience.Supplier,"/supplier")];
        foreach (var m in mods) if (!await db.TrainingModules.AnyAsync(x=>x.Code==m.code, ct)) { var tm=new TrainingModule(m.code,m.name,$"Overview, step-by-step instructions, role guidance and expected outcomes for {m.name}.",m.aud,m.name,30,"Published",now); tm.Lessons.Add(new TrainingLesson(tm.Id,"Overview",$"Use {m.route} to complete this ProcuraFlow activity. Expected outcome: trained user can perform the process and explain evidence points.",m.route,null,null,1)); tm.Lessons.Add(new TrainingLesson(tm.Id,"Step by step instructions","Follow seeded demo records, complete actions, and capture audit evidence.",m.route,null,null,2)); db.TrainingModules.Add(tm); }
        string[] phases=["Project Initiation","Discovery and Requirements Validation","Configuration","Integration","Data Migration","Testing and UAT","Training and Change Management","Go Live","Post Go Live Support"];
        for(int i=0;i<phases.Length;i++) if(!await db.ImplementationPhases.AnyAsync(x=>x.Name==phases[i],ct)){var ph=new ImplementationPhase($"PH-{i+1:00}",phases[i],$"Implementation phase: {phases[i]}",now.AddDays(i*7),now.AddDays(i*7+6),i<2?"Completed":"Planned"); var ms=new ImplementationMilestone(ph.Id,$"{phases[i]} milestone",$"Key outputs for {phases[i]}",now.AddDays(i*7+5),i<2?"Completed":"Planned","Implementation Lead",i<2?now:null); ms.Tasks.Add(new ImplementationTask(ms.Id,$"Complete {phases[i]} evidence",$"Prepare evidence and sign-off for {phases[i]}","Implementation Lead",i<2?"Completed":"Open",now.AddDays(i*7+4),i<2?now:null)); ph.Milestones.Add(ms); db.ImplementationPhases.Add(ph);}
        foreach (var sl in new[]{("CRIT","Critical",1,8,"Service Manager"),("HIGH","High",4,24,"Support Lead"),("MED","Medium",8,72,"Support Analyst"),("LOW","Low",24,120,"Support Analyst")}) if(!await db.SupportServiceLevels.AnyAsync(x=>x.Code==sl.Item1,ct)) db.SupportServiceLevels.Add(new SupportServiceLevel(sl.Item1,sl.Item2,sl.Item3,sl.Item4,sl.Item5,true));
        if(!await db.HandoverChecklists.AnyAsync(x=>x.Code=="PH1",ct)){var hc=new HandoverChecklist("PH1","Phase 1 Implementation Handover","Checklist covering system, security, UAT, training and support readiness.","Open",now,null); foreach(var cat in new[]{"System Configuration","Security Configuration","User Accounts","Roles and Permissions","Workflows","Business Rules","Supplier Data","Tender Data","Document Repository","Reporting","Integrations","Notifications","Backup Plan","Deployment Documentation","Training Completed","UAT Completed","Support Arrangements","Go Live Approval"}) hc.Items.Add(new HandoverChecklistItem(hc.Id,cat,$"Confirm {cat} is complete and evidence is linked.",true,"Open",$"/app/rfp-evidence",$"Evidence route for {cat}.",null,null)); db.HandoverChecklists.Add(hc);}
        await db.SaveChangesAsync(ct);
    }
    private static string RouteFor(string area) => area.Contains("Supplier")?"/app/suppliers":area.Contains("Tender")||area.Contains("Sourcing")?"/app/tenders":area.Contains("Requisition")?"/app/requisitions":area.Contains("Reporting")?"/app/reporting":area.Contains("Security")?"/app/security":area.Contains("Training")?"/app/training":"/app/rfp-evidence";
    private static string ApiFor(string area) => area.Contains("Supplier")?"/api/suppliers":area.Contains("Tender")?"/api/tenders":area.Contains("Requisition")?"/api/requisitions":area.Contains("Training")?"/api/training/modules":"/api/rfp-evidence/requirements";
    private static string DemoRoute(int step) => step switch {1=>"/app/dashboard",2=>"/app/planning",3=>"/app/requisitions",4=>"/app/tenders",5=>"/public/opportunities/LCA-RFP-2026-ICT-001",6=>"/public/opportunities/LCA-RFP-2026-ICT-001",7=>"/supplier/bids",8=>"/app/sealed-bids",9=>"/app/bid-openings",10=>"/app/evaluations",11=>"/app/awards",12=>"/app/purchase-orders",13=>"/app/contracts",14=>"/app/reporting",15=>"/app/audit",_=>"/app/studio"};
    private static string DemoRef(int step) => step switch {2=>"APP-2026-ICT",3=>"REQ-2026-ICT-001",4 or 5 or 6=>"LCA-RFP-2026-ICT-001",7=>"BID-2026-ICT-001",9=>"BO-2026-ICT-001",10=>"EVAL-2026-ICT-001",11=>"AWD-2026-ICT-001",12=>"PO-2026-ICT-001",13=>"CON-2026-ICT-001",_=>"LCA-DEMO-2026"};
    ...
    public static async Task SeedAsync(EProcurementDbContext db, CancellationToken cancellationToken = default)
    {
        if (db.Database.IsRelational())
        {
            ...
            await db.EnsureSealedBidSecuritySchemaAsync(cancellationToken);
            await db.EnsureBidOpeningSchemaAsync(cancellationToken);
            await db.EnsurePurchaseOrderManagementSchemaAsync(cancellationToken);
            ...
            await db.EnsureIdentitySecuritySchemaAsync(cancellationToken);
        }
        ...
        await AddNavigationLink(system.Id, "operations", "Operations", "/app/operations", "Activity", 60, @"[""Operations.View""]");
        await AddNavigationLink(system.Id, "support", "Support Cases", "/app/support", "LifeBuoy", 70, @"[""SupportCase.View""]");
        ...
        if (!await db.BackupPlans.AnyAsync(x => x.Code == "DAILY-DEMO", cancellationToken)) db.BackupPlans.Add(new BackupPlan("DAILY-DEMO", "Daily demo backup readiness", BackupType.Full, "Daily at 22:00 - simulated evidence only", "Configured by infrastructure", true, 30, DateTimeOffset.UtcNow, "system"));
        if (!await db.ApiPerformanceSamples.AnyAsync(cancellationToken)) db.ApiPerformanceSamples.Add(new ApiPerformanceSample("DEMO-SLOW-REQUEST", "/api/reporting/executive-dashboard", "GET", 200, 1250, "system", DateTimeOffset.UtcNow.AddMinutes(-15)));
        if (!await db.SupportCases.AnyAsync(cancellationToken)) db.SupportCases.Add(new SupportCase("SUP-DEMO-001", "Demo support readiness case", "Example case for go-live support tracking.", SupportCaseSeverity.Medium, SupportCaseStatus.Open, "Operations", "system", null, DateTimeOffset.UtcNow, DateTimeOffset.UtcNow, null, null, "DEMO-SUPPORT"));
        await db.SaveChangesAsync(cancellationToken);
        var plan = await db.BackupPlans.FirstAsync(x => x.Code == "DAILY-DEMO", cancellationToken);
        if (!await db.BackupRuns.AnyAsync(cancellationToken)) db.BackupRuns.Add(new BackupRun(plan.Id, DateTimeOffset.UtcNow.AddHours(-2), DateTimeOffset.UtcNow.AddHours(-2).AddMinutes(2), BackupRunStatus.Completed, "SIM-BACKUP-DEMO", 0, null, "system"));
        await db.SaveChangesAsync(cancellationToken);
        ...
        await SeedDataGovernanceAsync(db, cancellationToken);
        await db.SaveChangesAsync(cancellationToken);
    }
    private static async Task SeedDataGovernanceAsync(EProcurementDbContext db, CancellationToken ct)
    {
        if (!await db.DataGovernancePolicies.AnyAsync(x => x.Code == "LCA-DATA-OWNERSHIP", ct)) db.DataGovernancePolicies.Add(new DataGovernancePolicy("LCA-DATA-OWNERSHIP", "LCA Data Ownership Policy", "All procurement data, documents, audit records, supplier information and transaction history remain owned by Lesotho Communications Authority.", DataGovernancePolicyType.Ownership, "Platform", true, DateTimeOffset.UtcNow, null, "seed", DateTimeOffset.UtcNow));
        foreach (var e in new[] { "Supplier", "Tender", "BidSubmission", "BidOpeningSession", "EvaluationSession", "Award", "PurchaseOrder", "Contract", "SupplierInvoice", "AuditEvent", "DocumentRecord" }) if (!await db.DataRetentionRules.AnyAsync(x => x.EntityType == e, ct)) db.DataRetentionRules.Add(new DataRetentionRule($"RET-{e.ToUpperInvariant()}", $"Default {e} retention", e, 2555, 1825, null, true, true, true, "seed", DateTimeOffset.UtcNow));
        foreach (var p in new[] { ("Supplier","Email",DataPrivacyClassificationLevel.PersonalData), ("Supplier","PhoneNumber",DataPrivacyClassificationLevel.PersonalData), ("Supplier","TaxNumber",DataPrivacyClassificationLevel.Confidential), ("BidSubmissionDocument","Document",DataPrivacyClassificationLevel.SealedBidData), ("SupplierInvoice","TotalAmount",DataPrivacyClassificationLevel.FinancialData), ("AuditEvent","Actor",DataPrivacyClassificationLevel.AuditSensitive) }) if (!await db.DataPrivacyClassifications.AnyAsync(x => x.EntityType == p.Item1 && x.FieldName == p.Item2, ct)) db.DataPrivacyClassifications.Add(new DataPrivacyClassification(p.Item1, p.Item2, p.Item3, "Default LCA privacy classification", true, true, true, DateTimeOffset.UtcNow, "seed"));
        foreach (var c in new[] { ("SUP-MISSING-TAX","Supplier missing tax number","Supplier","Supplier.TaxNumber empty",MigrationIssueSeverity.Warning), ("SUP-MISSING-CATEGORY","Supplier missing category","Supplier","Supplier.Categories.Any() == false",MigrationIssueSeverity.Warning), ("TEN-CLOSING-BEFORE-PUBLICATION","Tender closing date before publication date","Tender","Tender.ClosingDate <= Tender.PublicationDate",MigrationIssueSeverity.Error) }) if (!await db.DataQualityChecks.AnyAsync(x => x.Code == c.Item1, ct)) db.DataQualityChecks.Add(new DataQualityCheck(c.Item1,c.Item2,c.Item3,c.Item2,c.Item4,c.Item5, true, DateTimeOffset.UtcNow));
        if (!await db.MigrationTemplates.AnyAsync(x => x.Code == "MIG-SUPPLIERS", ct)) { var t = new MigrationTemplate("MIG-SUPPLIERS", "Suppliers", "Supplier", "Historic supplier data", "CSV", true, "seed", DateTimeOffset.UtcNow); var fields = new[] { "SupplierReference", "LegalName", "TradingName", "RegistrationNumber", "TaxNumber", "Email", "PhoneNumber", "Address", "Category", "Status" }; for (var i=0;i<fields.Length;i++) t.Fields.Add(new MigrationTemplateField(t.Id, fields[i], fields[i], "String", i<2, 256, null, i==0?"SUP-HIST-001":fields[i], null, i+1)); db.MigrationTemplates.Add(t); }
        if (!await db.MigrationTemplates.AnyAsync(x => x.Code == "MIG-TENDERS", ct)) { var t = new MigrationTemplate("MIG-TENDERS", "Tenders", "Tender", "Historic tender data", "CSV", true, "seed", DateTimeOffset.UtcNow); var fields = new[] { "TenderNumber", "Title", "Description", "TenderType", "ProcurementMethod", "Category", "PublicationDate", "ClosingDate", "Status" }; for (var i=0;i<fields.Length;i++) t.Fields.Add(new MigrationTemplateField(t.Id, fields[i], fields[i], fields[i].Contains("Date")?"Date":"String", i<2, 256, null, i==0?"RFP-HIST-001":fields[i], null, i+1)); db.MigrationTemplates.Add(t); }
        if (!await db.MigrationTemplates.AnyAsync(x => x.Code == "MIG-PURCHASE-ORDERS", ct)) { var t = new MigrationTemplate("MIG-PURCHASE-ORDERS", "Purchase Orders", "PurchaseOrder", "Historic purchase order data", "CSV", true, "seed", DateTimeOffset.UtcNow); foreach (var f in new[] { "PurchaseOrderNumber", "SupplierReference", "IssueDate", "TotalAmount", "Status" }.Select((v,i)=>(v,i))) t.Fields.Add(new MigrationTemplateField(t.Id,f.v,f.v,f.v.Contains("Amount")?"Decimal":f.v.Contains("Date")?"Date":"String",f.i<2,256,null,f.v,null,f.i+1)); db.MigrationTemplates.Add(t); }
        if (!await db.DataExportRequests.AnyAsync(ct)) db.DataExportRequests.Add(new DataExportRequest("EXP-DEMO-001", "Supplier", "seed", DateTimeOffset.UtcNow, DataExportStatus.Completed, "CSV", false, "Demo completed export", DateTimeOffset.UtcNow, "exports/demo-suppliers.csv"));
        await db.SaveChangesAsync(ct);
    }
