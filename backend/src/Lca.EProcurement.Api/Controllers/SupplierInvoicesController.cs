...
public sealed class SupplierInvoicesController(ISupplierInvoiceApplicationService invoices) : ControllerBase
{
    ...
    [HttpPost("from-purchase-order/{purchaseOrderId:guid}")] public async Task<IActionResult> FromPo(Guid purchaseOrderId,AppActorDto dto,CancellationToken ct){var i=await invoices.CreateFromPurchaseOrderAsync(purchaseOrderId,dto,ct); return Created($"/api/invoices/{i.Id}",i);}
    [HttpPost("{id:guid}/submit")] public async Task<IActionResult> Submit(Guid id,AppActorDto dto,CancellationToken ct)=>(await invoices.SubmitAsync(id,dto,ct)) is { } i?Ok(i):NotFound();
    [HttpPost("{id:guid}/run-match")] public async Task<IActionResult> RunMatch(Guid id,AppActorDto dto,CancellationToken ct)=>(await invoices.RunMatchAsync(id,dto,ct)) is { } i?Ok(i):NotFound();
    [HttpPost("{id:guid}/approve-for-payment")] public async Task<IActionResult> Approve(Guid id,AppActorDto dto,CancellationToken ct)=>(await invoices.ApproveForPaymentAsync(id,dto,ct)) is { } i?Ok(i):NotFound();
    ...
    [HttpPost("{id:guid}/cancel")] public async Task<IActionResult> Cancel(Guid id,AppActorDto dto,CancellationToken ct)=>(await invoices.CancelAsync(id,dto,ct)) is { } i?Ok(i):NotFound();
