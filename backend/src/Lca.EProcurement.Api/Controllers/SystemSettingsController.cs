...
public sealed class SystemSettingsController(ISystemSettingsApplicationService settings) : ControllerBase
{
    ...
    [Authorize(Policy = "Settings.Manage")]
    [HttpPost("test-sms")]
    public Task<IActionResult> TestSms(Dictionary<string,string> dto, CancellationToken ct) => OkAsync(settings.TestSmsSettingsAsync(dto.GetValueOrDefault("phoneNumber") ?? "+26600000000", ct));
