...
import { createClarification, createTender, getClarificationWorkspace, getTender, getTenders, publishTender, respondClarification, TenderDetail, TenderSummary } from "../../services/tendersApi";
...
export function TenderDetailPage({ id }: { id: string }) {
  ...
  const isPublished = t.status === "Published" || Boolean(t.publishedAt);
  const publicUrl = `/public/opportunities/${encodeURIComponent(t.tenderNumber)}`;
  const publicDocumentsCount = detail.documents.filter(d => d.isPublic && d.isDownloadable).length;
  const publicClarificationsCount = detail.clarifications.filter(c => c.isPublic && c.responses?.length).length;
  return <><PageHeader title={t.title} description={`${t.tenderNumber} · ${t.tenderType} · ${t.procurementMethod}`} actions={<>{t.status === "Draft" && <Button onClick={async () => { await publishTender(id); load(); }}>Publish</Button>}<Button variant="secondary" onClick={() => navigate("/app/tenders")}>Back</Button></>} /><div className="row g-3"><div className="col-lg-8"><div className="card"><div className="card-body"><Badge>{t.status}</Badge><p className="mt-3">{t.description}</p><dl className="row"><dt className="col-sm-3">Closing</dt><dd className="col-sm-9">{new Date(t.closingDate).toLocaleString()}</dd><dt className="col-sm-3">Published</dt><dd className="col-sm-9">{t.publishedAt ? new Date(t.publishedAt).toLocaleString() : "Not published"}</dd></dl></div></div><Clarifications detail={detail} question={question} setQuestion={setQuestion} responses={responses} setResponses={setResponses} reload={load} /></div><div className="col-lg-4">{isPublished && <PublicPublicationPanel publicUrl={publicUrl} publishedAt={t.publishedAt} publishedBy={t.publishedBy} publicDocumentsCount={publicDocumentsCount} publicClarificationsCount={publicClarificationsCount} />}<div className="card"><div className="card-header">Documents</div><div className="list-group list-group-flush">{detail.documents.map(d => <div className="list-group-item" key={d.id}><strong>{d.documentType}</strong><div>{d.fileName}</div><small>{d.description}</small></div>)}</div></div><div className="card mt-3"><div className="card-header">Audit</div><div className="card-body">{detail.auditTimeline.map(a => <p key={a.id}><strong>{a.eventType}</strong><br/><small>{a.actor} · {new Date(a.occurredAt).toLocaleString()}</small></p>)}</div></div></div></div></>;
}
function PublicPublicationPanel({ publicUrl, publishedAt, publishedBy, publicDocumentsCount, publicClarificationsCount }: { publicUrl: string; publishedAt?: string | null; publishedBy?: string | null; publicDocumentsCount: number; publicClarificationsCount: number }) {
  return <div className="card mb-3" data-testid="public-publication-panel"><div className="card-header">Public Publication</div><div className="card-body"><dl className="row mb-3"><dt className="col-sm-5">Public visibility</dt><dd className="col-sm-7"><Badge>Visible</Badge></dd><dt className="col-sm-5">Public URL</dt><dd className="col-sm-7"><code>{publicUrl}</code></dd><dt className="col-sm-5">Published date</dt><dd className="col-sm-7">{publishedAt ? new Date(publishedAt).toLocaleString() : "Not published"}</dd><dt className="col-sm-5">Published by</dt><dd className="col-sm-7">{publishedBy || "Not recorded"}</dd><dt className="col-sm-5">Public documents count</dt><dd className="col-sm-7">{publicDocumentsCount}</dd><dt className="col-sm-5">Public clarifications count</dt><dd className="col-sm-7">{publicClarificationsCount}</dd></dl><Button variant="secondary" onClick={() => window.open(publicUrl, "_blank", "noopener,noreferrer")}>View Public Notice</Button></div></div>;
}

function Clarifications({ detail, question, setQuestion, responses, setResponses, reload }: any) { return <div className="card mt-3"><div className="card-header">Clarification workspace</div><div className="card-body"><form onSubmit={async e => { e.preventDefault(); await createClarification(detail.tender.id, question, "supplier@demo.co.ls"); setQuestion(""); reload(); }} className="input-group mb-3"><input className="form-control" value={question} onChange={e => setQuestion(e.target.value)} placeholder="Ask a clarification question" required /><Button>Ask</Button></form>{detail.clarifications.map((c: any) => <div className="border rounded p-3 mb-2" key={c.id}><strong>{c.question}</strong><div><small>{c.askedBy} · {new Date(c.askedAt).toLocaleString()}</small></div>{c.responses.map((r: any) => <div className="alert alert-info mt-2 mb-0" key={r.id}>{r.response}<br/><small>{r.respondedBy}</small></div>)}<form onSubmit={async e => { e.preventDefault(); await respondClarification(detail.tender.id, c.id, responses[c.id] || "", c.visibility === "Public" || c.isPublic, c.visibility === "Public" || c.isPublic); setResponses({ ...responses, [c.id]: "" }); reload(); }} className="input-group mt-2"><input className="form-control" value={responses[c.id] || ""} onChange={e => setResponses({ ...responses, [c.id]: e.target.value })} placeholder="Response" required /><Button variant="secondary">Respond</Button></form></div>)}</div></div>; }

export function TenderClarificationsWorkspacePage() {
  const [items, setItems] = useState<any[]>([]);
  const [loading, setLoading] = useState(true);
  useEffect(() => { getClarificationWorkspace().then(r => { setItems(r.data || []); setLoading(false); }); }, []);
  if (loading) return <LoadingState />;
  return <><PageHeader title="Tender clarifications" description="Respond, classify and publish supplier clarification responses." />
    <div className="card"><div className="card-body table-responsive"><table className="table table-hover"><thead><tr><th>Tender</th><th>Question reference</th><th>Supplier</th><th>Asked date</th><th>Status</th><th>Visibility</th><th>Response status</th><th>Assigned officer</th><th>Actions</th></tr></thead><tbody>
      {items.map(c => <tr key={c.id}><td>{c.tenderNumber}<br/><small>{c.tenderTitle}</small></td><td>{c.questionReference}</td><td>{c.supplierName}</td><td>{new Date(c.askedAt).toLocaleString()}</td><td><Badge>{c.status}</Badge></td><td>{c.visibility}</td><td>{c.responseStatus}</td><td>{c.assignedOfficer || "Unassigned"}</td><td><Button variant="secondary" onClick={() => navigate(`/app/tenders/${c.tenderId}/clarifications`)}>Respond</Button></td></tr>)}
    </tbody></table>{items.length === 0 && <EmptyState title="No clarifications" message="Supplier clarification questions will appear here." />}</div></div></>;
}
