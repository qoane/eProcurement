...
import { archivePageDefinition, createPageDefinition, getPageDataSources, getPageDefinitions, publishPageDefinition, updatePageDefinition } from "../../services/pageDefinitionsApi";
import type { PageComponent, PageDataSourceOption, PageDesigner, PageType } from "../../types/api";
...
export function PageDesignerPage() {
  const [pages, setPages] = useState<PageDesigner[]>([]); const [dataSources, setDataSources] = useState<PageDataSourceOption[]>([]); const [form, setForm] = useState<PageDesigner>(blank); const [selectedCode, setSelectedCode] = useState("page-header"); const [message, setMessage] = useState(""); const [preview, setPreview] = useState(false);
  const components = form.components ?? [];
  const selectedComponent = components.find((x) => x.code === selectedCode) || components[0];
  async function load() {
    const [pageResult, dataSourceResult] = await Promise.all([getPageDefinitions(), getPageDataSources()]);
  ...
  const selectedDataSource = dataSources.find((source) => source.entity === form.datasource.entity && (source.listEndpoint || "") === (form.datasource.endpoint || ""));
  function applyDataSource(code: string) {
    const source = dataSources.find((item) => item.code === code);
    if (!source) return;
    setForm({
      ...form,
      datasource: { entity: source.entity, mode: source.mode, endpoint: source.listEndpoint, keyField: source.keyField },
    });
  }
  async function save() {
  ...
  function patchComponent(patch: Partial<PageComponent>, configuration?: Record<string, string>) { setForm({ ...form, components: components.map((c) => c.code === selectedComponent?.code ? { ...c, ...patch, configuration: configuration ? { ...configOf(c), ...configuration } : c.configuration } : c) }); }
  ...
  return <div className="studio-page page-designer">
    ...
    <div className="grid cols-3">
      ...
      <Card><h2>Canvas</h2><div className="form-grid"><label>Name<Input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} /></label><label>Code<Input value={form.code} onChange={(e) => setForm({ ...form, code: slug(e.target.value) })} /></label><label>Application<Input placeholder="ApplicationId" value={form.applicationId || ""} onChange={(e) => setForm({ ...form, applicationId: e.target.value || undefined })} /></label><label>Route<Input value={form.route} onChange={(e) => setForm({ ...form, route: e.target.value, navigation: { ...form.navigation, route: e.target.value } })} /></label><label>Page Type<Select value={form.pageType} onChange={(e) => setForm({ ...form, pageType: e.target.value as PageType })}>{pageTypes.map((x) => <option key={x}>{x}</option>)}</Select></label><label>Datasource<Select value={selectedDataSource?.code || "custom"} onChange={(e) => applyDataSource(e.target.value)}><option value="custom">Custom / manual datasource</option>{dataSources.map((source) => <option key={source.code} value={source.code}>{source.label} ({source.mode})</option>)}</Select><small>{selectedDataSource?.description || "Choose a generated metadata datasource or custom API-backed datasource discovered from existing page definitions."}</small></label><label>Endpoint<Input value={form.datasource.endpoint || ""} onChange={(e) => setForm({ ...form, datasource: { ...form.datasource, endpoint: e.target.value || undefined, mode: form.datasource.mode || "CustomApi" } })} /></label><label>Key field<Input value={form.datasource.keyField || "id"} onChange={(e) => setForm({ ...form, datasource: { ...form.datasource, keyField: e.target.value || "id" } })} /></label><label>Datasource entity<Input value={form.datasource.entity} onChange={(e) => setForm({ ...form, datasource: { ...form.datasource, entity: e.target.value } })} /></label><label>Icon<Input value={form.icon} onChange={(e) => setForm({ ...form, icon: e.target.value })} /></label><label>Permissions<Input value={form.permissions.map((p) => p.role).join(", ")} onChange={(e) => setForm({ ...form, permissions: e.target.value.split(",").map((role) => ({ role: role.trim(), access: "View" })).filter((p) => p.role) })} /></label></div><label>Description<textarea className="input" value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} /></label><h3>Layout</h3><div className="grid">{layouts.map((l) => <Button key={l.template} variant={form.layout.template === l.template ? "primary" : "secondary"} onClick={() => setForm({ ...form, layout: { ...form.layout, template: l.template, regions: l.regions } })}>{l.template}</Button>)}</div><div className="designer-canvas">{regions.map((region) => <section className="panel" key={region}><h3>{region}</h3>{components.filter((c) => c.region === region).sort((a,b)=>(a.displayOrder||0)-(b.displayOrder||0)).map((c) => <button className="input" key={c.code} onClick={() => setSelectedCode(c.code)}><strong>{c.name}</strong><br/><span>{c.componentType}</span></button>)}{!components.some((c) => c.region === region) && <EmptyState title="Region ready" message="Add components from the toolbox to compose this page region." />}</section>)}</div></Card>
